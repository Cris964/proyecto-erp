...
const app = express();
const SECRET = 'AccuCloud_2026_Ultra_Premium_Key';
...
const q = async (sql, params) => {
    ...
    try {
        ...
        return rows;
    } catch (e) { console.error("❌ Error DB:", e.message); return []; }
...
app.get('/api/dashboard-data', auth, async (req, res) => {
    ...
    res.json({ cajaMayor: parseFloat(v[0].t) + parseFloat(b[0].t), topProducts: top, lowStock: l[0].c });
});
...
app.post('/api/productos', auth, async (req, res) => {
    const { sku, nombre, fecha_vencimiento, costo_compra, costo_venta, stock, bodega_id } = req.body;
    await q("INSERT INTO productos (company_id, sku, nombre, fecha_vencimiento, costo_compra, costo_venta, stock, min_stock, bodega_id) VALUES (?,?,?,?,?,?,?,5,?)", 
    [req.user.company_id, sku, nombre, fecha_vencimiento, costo_compra, costo_venta, stock, bodega_id]);
...
app.post('/api/empleados', auth, async (req, res) => {
    ...
    await q("INSERT INTO empleados (company_id, nombre, documento, valor_dia, hire_date, eps, arl, pension) VALUES (?,?,?,?,?,?,?,?)", 
    [req.user.company_id, nombre, documento, valor_dia, hire_date, eps, arl, pension]);
    res.json({ success: true });
});
...
app.get('/api/produccion/ordenes', auth, async (req, res) => res.json(await q("SELECT o.*, r.nombre_kit FROM ordenes_produccion o JOIN recetas r ON o.receta_id=r.id WHERE o.company_id=?", [req.user.company_id])));
...
app.get('/api/caja-menor', auth, async (req, res) => res.json(await q("SELECT * FROM caja_menor_movs WHERE company_id=?", [req.user.company_id])));
