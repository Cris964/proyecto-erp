...
app.post('/api/nomina/liquidar', async (req, res) => {
    ...
    try {
        ...
        const SMLV = 1750905; const AUX = 249095; const S = parseFloat(emp.salario);
        const basico = Math.round((S / 30) * dias);
        const auxilio = (S <= SMLV * 2) ? Math.round((AUX / 30) * dias) : 0;
        ...
        try {
            await transporter.sendMail({
                ...
                html: `<h1>Pago de $${neto.toLocaleString()}</h1><p>Hola ${emp.nombre}, tu nómina ha sido procesada.</p>`
            });
...
app.post('/api/ventas', async (req, res) => {
    ...
    try {
        ...
        if (es_electronica && cliente?.email) {
            try { await transporter.sendMail({ from: '"Facturación AccuCloud" <crisplusplay@gmail.com>', to: cliente.email, subject: `Factura Venta`, html: `<h1>Compra registrada exitosamente</h1>` }); } catch(e){}
        }
        await c.commit();
        res.json({ success: true });
