...
app.post('/api/kits', async (req, res) => {
    try {
        const { nombre, precio, productos, company_id } = req.body;
        const [resK] = await pool.query("INSERT INTO kits (nombre, precio, company_id) VALUES (?,?,?)", [nombre, precio, company_id]);
        res.json({ success: true });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/dashboard-data', async (req, res) => {
    try {
        ...
        res.json({ cajaMayor: v[0].total, cajaMenor: 0, valorInventario: p[0].valor, lowStock: c[0].total });
    } catch (e) { res.json({ cajaMayor: 0, cajaMenor: 0, valorInventario: 0, lowStock: 0 }); }
});
