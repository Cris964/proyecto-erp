...
app.post('/api/empleados', verificarToken, async (req, res) => {
    const { nombre, documento, salario, eps, arl, pension, cargo } = req.body;
    await q("INSERT INTO empleados (nombre, documento, salario, eps, arl, pension, cargo, company_id) VALUES (?,?,?,?,?,?,?,?)", [nombre, documento, salario || 0, eps, arl, pension, cargo || 'Operario', req.user.company_id]);
    res.json({ success: true });
});
