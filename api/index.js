...
const app = express();
const SECRET = 'AccuCloud_Ultimate_2026_Final_Version';
...
app.post('/api/login', async (req, res) => {
    ...
    if (r.length > 0) {
        const token = jwt.sign({ id: r[0].id, company_id: r[0].company_id, cargo: r[0].cargo, nombre: r[0].nombre }, SECRET, {expiresIn: '24h'});
...
app.post('/api/produccion/ordenes', auth, async (req, res) => {
    ...
    await q("INSERT INTO ordenes_produccion (company_id, numero_orden, receta_id, cantidad_requerida, usuario_monta) VALUES (?,?,?,?,?)", [req.user.company_id, 'ORD-'+Date.now(), receta_id, cantidad, req.user.nombre]);
    res.json({ success: true });
});
