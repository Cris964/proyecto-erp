...
import { 
  ...
  User, Moon, Sun, Camera, Search, TrendingUp, ChevronRight, Box, ClipboardList
} from 'lucide-react';
...
export default function App() {
  ...
  if (loading) return <div className="h-screen flex items-center justify-center font-black animate-pulse text-blue-600">INICIANDO ACCUCLOUD PRO...</div>;
      ...
      <div className="flex h-screen bg-slate-50 dark:bg-slate-950 font-sans overflow-hidden transition-all duration-300">
        ...
        <aside className="w-80 bg-white dark:bg-slate-900 border-r dark:border-slate-800 flex flex-col p-8 shadow-2xl z-20">
          ...
          <div className="pt-6 border-t dark:border-slate-800 mt-6">
              ...
              <button onClick={()=>{localStorage.clear(); window.location.reload();}} className="text-red-500 font-bold text-xs uppercase p-4 rounded-2xl w-full hover:bg-red-50 dark:hover:bg-red-900/20 transition">Salir</button>
          </div>
        </aside>
...
function DashboardView() {
    ...
    return (
        <div className="space-y-12">
            ...
            <div className="bg-white dark:bg-slate-900 p-12 rounded-[50px] shadow-sm border dark:border-slate-800">
                <h3 className="font-black text-xl mb-8 flex items-center gap-3 text-slate-400 uppercase text-[10px] tracking-widest"><TrendingUp className="text-blue-600"/> Top Ventas</h3>
                <div className="space-y-4">
                    {data.topProducts.map((p, i) => (
                        <div key={i} className="flex justify-between items-center p-5 bg-slate-50 dark:bg-slate-800 rounded-[30px] border dark:border-slate-700 hover:scale-[1.01] transition-transform">
        ...
        <div className="grid grid-cols-3 gap-10 h-full">
            <div className="col-span-2 space-y-6">
                <div className="bg-white dark:bg-slate-900 p-6 rounded-[30px] border dark:border-slate-800 shadow-sm flex items-center gap-4">
                    ...
                    <input className="w-full bg-transparent font-bold outline-none dark:text-white text-lg" placeholder="Buscar..." onChange={e=>setSearch(e.target.value)} />
                </div>
            ...
            <div className="bg-white dark:bg-slate-900 p-10 rounded-[60px] shadow-2xl border-t-[20px] border-blue-600 h-fit sticky top-0">
        ...
        <div className="space-y-8">
            ...
            {sub==='recetas' && (
                <div className="bg-white dark:bg-slate-900 p-12 rounded-[50px] border dark:border-slate-800 shadow-2xl">
                    ...
                    <div className="grid grid-cols-2 gap-10">
                        ...
                        <button onClick={async()=>{ const n = prompt("Nombre?"); await axios.post('/api/produccion/recetas', {nombre:n, descripcion:'...'}); load(); }} className="p-10 border-4 border-dotted rounded-[45px] flex items-center justify-center text-slate-300 font-black hover:bg-slate-50 transition-all">+ Nueva Receta</button>
                    </div>
                </div>
            )}
            ...
            {sub==='ordenes' && (
                <div className="space-y-10">
                    <div className="bg-white dark:bg-slate-900 p-10 rounded-[50px] border shadow-2xl flex gap-8 items-end border-t-[15px] border-blue-600">
                        ...
                        <button onClick={async()=>{ await axios.post('/api/produccion/ordenes', {receta_id:document.getElementById('rI').value, cantidad:document.getElementById('rC').value}); load(); alert("Orden Montada"); }} className="bg-blue-600 text-white px-12 py-5 rounded-3xl font-black shadow-xl">Lanzar Órden</button>
                    </div>
                    {ordenes.map(o => (
                        <div key={o.id} className="bg-white dark:bg-slate-900 p-10 rounded-[45px] border dark:border-slate-800 shadow-sm flex justify-between items-center group hover:border-blue-500 transition-all">
                            <div><h4 className="text-2xl font-black text-slate-900 dark:text-white italic">{o.numero_orden} - {o.nombre_kit}</h4><p className="text-[10px] font-bold text-slate-400 uppercase mt-3">Montó: {o.usuario_monta}</p></div>
                            <div className="flex items-center gap-8">
                                <div className="text-right"><p className="text-[10px] font-black text-slate-400 mb-1 uppercase">Requerido</p><p className="font-black text-2xl dark:text-white">{o.cantidad_requerida}</p></div>
...
function NominaView() {
    ...
    const [fN, setFN] = useState({ nombre:'', documento:'', valor_dia:0, hire_date:'', eps:'', arl:'', pension:'' });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 h-fit">
            <form onSubmit={async(e)=>{e.preventDefault(); await axios.post('/api/empleados', fN); alert("Vinculado"); load();}} className="bg-white dark:bg-slate-900 p-12 rounded-[50px] border dark:border-slate-800 shadow-2xl space-y-6 h-fit">
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <input className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl font-bold border-none dark:text-white" type="date" onChange={e=>setFN({...fN, hire_date:e.target.value})}/>
                </div>
            ...
            <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-8 h-fit">
                {emps.map(e => (
                    <div key={e.id} className="bg-white dark:bg-slate-900 p-10 rounded-[45px] border dark:border-slate-800 shadow-sm flex flex-col gap-6 group hover:border-blue-600 transition-all">
                        ...
                        <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-800 p-6 rounded-[30px]">
                            <div><p className="text-[9px] font-black text-slate-400 uppercase">Salario Día</p><p className="text-3xl font-black text-green-600">{fmt(e.valor_dia)}</p></div>
                            <button onClick={()=>alert(`Liquidando a ${e.nombre}... Total Mes (30d): ${fmt(e.valor_dia * 30)}`)} className="bg-slate-900 dark:bg-blue-600 text-white px-8 py-3 rounded-2xl font-black text-[10px] uppercase shadow-lg">LIQUIDAR</button>
                        </div>
                    </div>
                ))}
            </div>
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 animate-fade-in h-fit">
            <div className="bg-white dark:bg-slate-900 p-12 rounded-[60px] border dark:border-slate-800 shadow-2xl space-y-10">
                ...
                <form onSubmit={async(e)=>{e.preventDefault(); await axios.put('/api/perfil', f); alert("Perfil Actualizado"); setUser({...user, ...f});}} className="space-y-6">
                    <input className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" value={f.nombre} placeholder="Nombre Público" onChange={e=>setF({...f, nombre:e.target.value})} />
                    <input className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" value={f.avatar_url} placeholder="URL Foto Perfil" onChange={e=>setF({...f, avatar_url:e.target.value})} />
            ...
            <div className="bg-blue-600 p-12 rounded-[60px] text-white shadow-2xl h-fit">
                ...
                <button onClick={()=>window.open('https://wa.me/57321654987')} className="bg-white text-blue-600 px-8 py-4 rounded-3xl font-black uppercase text-xs">WhatsApp Soporte</button>
            </div>
        </div>
    );
}
            ...
            <div className="bg-white dark:bg-slate-900 p-20 rounded-[60px] border dark:border-slate-800 shadow-2xl border-t-[20px] border-blue-600">
                ...
                {activo ? (
                    <div className="space-y-8 animate-fade-in text-left">
                        ...
                        <div className="grid grid-cols-2 gap-6">
                            <div className="bg-slate-50 dark:bg-slate-800 p-8 rounded-[40px] border dark:border-slate-700"><p className="text-[10px] font-black text-slate-400 uppercase mb-2">Base</p><p className="text-3xl font-black text-slate-600 dark:text-white">{fmt(activo.base_caja)}</p></div>
...
function PagosView() { return null; } // Solo para evitar error, el componente real se integra arriba.