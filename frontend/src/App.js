...
function NominaIndustrial({ user }) {
    ...
    const handleSave = async (e) => {
        ...
        } catch (e) { window.alert("Error al guardar colaborador."); }
    };

    const simular = (e) => {
        const sal = parseFloat(e.salario);
        const aux = (sal <= 3501810) ? 249095 : 0; // Aux Transporte 2026
        const salud = sal * 0.04; const pens = sal * 0.04;
        const neto = (sal + aux) - (salud + pens);
        setPreview({ ...e, neto, aux, sal, salud, pens });
    };
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            ...
            {tab === 'add' && (
                <div className="bg-white p-12 rounded-[60px] shadow-sm border border-slate-100 max-w-4xl">
                    ...
                    <form onSubmit={handleSave} className="grid grid-cols-2 gap-8">
                        ...
                        <button className="col-span-2 py-8 bg-blue-600 text-white font-black rounded-3xl shadow-xl uppercase tracking-widest active:scale-95 transition-all mt-6">REGISTRAR Y ACTIVAR NÓMINA</button>
                    </form>
                </div>
            )}
            ...
            {tab === 'list' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    <div className="bg-white rounded-[50px] shadow-sm border border-slate-100 overflow-hidden pr-2 h-fit">
                        <table className="w-full text-left">
                            <thead className="bg-slate-50 text-[10px] font-black uppercase text-slate-400 border-b tracking-widest"><tr><th className="p-10">Colaborador</th><th>Salario Base</th><th className="text-center">Simular</th></tr></thead>
                            <tbody>{empleados.map(e=>(
                                <tr key={e.id} className="border-b hover:bg-slate-50 transition">
                                    ...
                                    <td className="text-center"><button onClick={()=>simular(e)} className="p-4 bg-blue-50 text-blue-600 rounded-2xl hover:bg-blue-600 hover:text-white transition-all shadow-sm"><Calculator/></button></td>
                                </tr>
                            ))}</tbody>
                        </table>
                    </div>
...
function ProduccionIndustrial({ user }) {
    ...
    const handleAvanzar = async (id, estado) => {
        let logistica = {};
        if (estado === 'Cerrado') {
            const ciudad = window.prompt("Ciudad de envío:");
            const dir = window.prompt("Dirección de entrega:");
            logistica = { ciudad, dir, transportadora: 'Envía', fecha: new Date() };
        }
        await axios.put(`/api/produccion/ordenes/${id}`, { estado, datos_logistica: logistica });
        load();
    };
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            ...
            {sub === 'materia' && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                    <div className="bg-white p-12 rounded-[60px] shadow-sm border border-slate-100 h-fit">
                        <h3 className="font-black text-2xl mb-8 uppercase italic flex items-center gap-4 text-slate-800 tracking-tighter underline decoration-blue-500 decoration-8 underline-offset-8"><Database className="text-blue-600" size={32}/> Insumos Industriales</h3>
                    ...
                    <div className="lg:col-span-2 bg-white rounded-[60px] shadow-sm border border-slate-100 overflow-hidden pr-2 h-fit">
                        <table className="w-full text-left">
            ...
            {sub === 'kits' && (
                <div className="bg-slate-900 p-20 rounded-[80px] shadow-2xl relative overflow-hidden text-white animate-slide-up">
                    ...
                    <div className="max-w-2xl relative z-10">
                        ...
                        <p className="text-slate-400 font-bold mb-12 text-lg italic leading-relaxed">Genera combos automáticos seleccionando múltiples productos para crear un SKU especial sincronizado con el stock técnico.</p>
                        <form className="space-y-6">
                            ...
                            <button className="px-16 py-8 bg-blue-600 rounded-[40px] font-black shadow-xl uppercase text-sm tracking-widest active:scale-95 transition-all hover:bg-white hover:text-blue-600">+ Generar Nuevo SKU de Kit Corporativo</button>
                        </form>
                    </div>
                </div>
            )}
                ...
                <div className="space-y-10">
                    <div className="bg-blue-600 p-16 rounded-[80px] text-white flex flex-col md:flex-row justify-between items-center shadow-2xl relative overflow-hidden">
                        ...
                        <div className="relative z-10 text-center md:text-left">
                            <p className="text-[12px] font-black uppercase tracking-[6px] mb-4 opacity-70 tracking-widest leading-none">Pipeline de Fabricación Industrial</p>
                        ...
                        {(ordenes || []).filter(o => ['Prealistamiento', 'Produccion'].includes(o.estado)).map(o => (
                            <div key={o.id} className="bg-white p-12 rounded-[70px] shadow-md border-l-[30px] border-blue-500 flex flex-col lg:flex-row justify-between items-center transition-all hover:shadow-2xl group">
                                ...
                                <div className="flex gap-6 mt-10 lg:mt-0">
                                    {o.estado === 'Prealistamiento' && <button onClick={()=>handleAvanzar(o.id, 'Produccion')} className="flex items-center gap-4 px-12 py-6 bg-slate-900 text-white font-black rounded-[35px] shadow-2xl hover:bg-black transition-all uppercase text-xs tracking-widest active:scale-95"><Play size={24}/> Iniciar Mezcla</button>}
                                    {o.estado === 'Produccion' && <button onClick={()=>handleAvanzar(o.id, 'Logistica')} className="flex items-center gap-4 px-12 py-6 bg-green-600 text-white font-black rounded-[35px] shadow-2xl hover:bg-green-700 transition-all uppercase text-xs tracking-widest active:scale-95"><CheckCircle size={24}/> Sellar Batch</button>}
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            {sub === 'logistica' && (
                <div className="bg-white rounded-[80px] shadow-sm border border-slate-100 overflow-hidden pr-2 animate-fade-in h-fit">
                    <table className="w-full text-left">
                        <thead className="bg-slate-900 text-white text-[11px] font-black uppercase tracking-[4px]"><tr><th className="p-12">Batch #</th><th>Producto Final</th><th>Operación</th></tr></thead>
                        <tbody>{ordenes.filter(o => o.estado === 'Logistica').map(o => (
                            <tr key={o.id} className="border-b hover:bg-slate-50 transition-all">
                                <td className="p-12 font-black text-blue-600 text-2xl italic tracking-tighter uppercase leading-none">OP-{o.numero_orden}</td>
                                <td className="font-black text-slate-800 text-xl tracking-tighter uppercase italic leading-none">{o.nombre_producto}</td>
                                <td className="p-12 text-right">
                                    <button onClick={()=>handleAvanzar(o.id, 'Cerrado')} className="px-10 py-4 bg-blue-600 text-white font-black rounded-[25px] shadow-xl uppercase text-xs tracking-[4px] active:scale-95">Despachar</button>
                                </td>
                            </tr>
                        ))}</tbody>
                    </table>
                    {ordenes.filter(o=>o.estado==='Logistica').length === 0 && <div className="p-32 text-center opacity-20 font-black italic text-4xl uppercase tracking-tighter">Sin pedidos en despacho</div>}
                </div>
            )}
        </div>
    );
...
function InventarioIndustrial({ user }) {
    ...
    return (
        <div className="space-y-10 animate-fade-in">
            <div className="flex gap-4 p-2 bg-white border rounded-[30px] w-fit shadow-sm overflow-x-auto">
                <button onClick={()=>setTab('list')} className={`px-10 py-3 rounded-2xl font-black text-[10px] uppercase transition-all ${tab==='list'?'bg-blue-600 text-white shadow-xl shadow-blue-200':'text-slate-400'}`}>CONSULTAR EXISTENCIAS</button>
                <button onClick={()=>setTab('new')} className={`px-10 py-3 rounded-2xl font-black text-[10px] uppercase transition-all ${tab==='new'?'bg-blue-600 text-white shadow-xl shadow-blue-200':'text-slate-400'}`}>+ CREACIÓN MANUAL</button>
                <button onClick={()=>setTab('bodegas')} className={`px-10 py-3 rounded-2xl font-black text-[10px] uppercase transition-all ${tab==='bodegas'?'bg-blue-600 text-white shadow-xl shadow-blue-200':'text-slate-400'}`}>BODEGAS</button>
            ...
            {tab === 'new' && (
                <div className="bg-white p-16 rounded-[75px] shadow-sm border border-slate-100 max-w-4xl animate-slide-up relative">
                    ...
                    <form onSubmit={handleSave} className="grid grid-cols-2 gap-10">
                        ...
                        <button className="col-span-2 py-8 bg-blue-600 text-white font-black rounded-[35px] shadow-xl uppercase tracking-widest active:scale-95 transition-all text-xs tracking-[3px]">Sincronizar al Almacén</button>
                    </form>
                </div>
            )}
            ...
            {tab === 'bodegas' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    <div className="bg-white p-12 rounded-[60px] shadow-sm border border-slate-100 h-fit">
                        <h3 className="font-black text-2xl mb-8 uppercase italic tracking-tighter text-slate-800 underline decoration-blue-500 decoration-4 underline-offset-8 leading-none">Crear Zona Logística</h3>
            ...
            {tab === 'list' && (
                <div className="bg-white rounded-[65px] shadow-sm border border-slate-100 overflow-hidden pr-2 animate-fade-in">
                    <table className="w-full text-left">
                        <thead className="bg-slate-50 text-[10px] font-black uppercase border-b text-slate-400 tracking-[5px] leading-none"><tr><th className="p-12">Descripción Item</th><th>Batch ID</th><th>Existencias</th><th>Estado Alerta</th><th className="text-center p-12">Control</th></tr></thead>
        ...
        <div className="space-y-12 animate-fade-in">
            <div className="bg-white p-16 rounded-[80px] shadow-sm border border-slate-100 relative overflow-hidden">
                ...
                <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-5 gap-8 relative z-10">
                    ...
                    <div className="space-y-2"><label className="text-[10px] font-black uppercase text-slate-400 ml-6 tracking-widest leading-none">Rol en Sistema</label><select className="w-full p-6 bg-slate-50 rounded-[35px] font-black text-slate-700 outline-none focus:ring-4 ring-blue-50 text-sm leading-none" value={form.cargo} onChange={e=>setForm({...form, cargo: e.target.value})}>
                    ...
                    <tbody>{(usuarios || []).map(u => (
                        <tr key={u.id} className="border-b hover:bg-slate-50 transition-all">
                            ...
                            <td className="p-12 text-center flex justify-center gap-6 leading-none">
                                ...
                                <button onClick={async ()=>{if(window.confirm("¿Dar de baja acceso?")){ await axios.delete(`/api/admin/usuarios/${u.id}`); load(); }}} className="p-5 bg-red-50 text-red-500 rounded-[25px] hover:bg-red-600 hover:text-white transition-all shadow-sm"><Trash2 size={24}/></button>
                            </td>
                        </tr>
                    ))}</tbody>
                </table>
...
function CajaMasterView({ user, turnoActivo, onUpdate }) {
    const handleApertura = async () => {
        const pass = window.prompt("ACCESO CORPORATIVO: Introduce la CLAVE MAESTRA corporativa (admin123):");
        if (!pass) return;
        try {
            ...
            {turnoActivo ? (
                <div className="space-y-12 animate-fade-in">
                    <div className="p-16 bg-slate-50 rounded-[60px] border-2 border-dashed border-slate-200 shadow-inner">
                        <p className="text-[12px] font-black text-slate-400 uppercase tracking-[8px] mb-6 leading-none">Monto Neto en Caja</p>
                    ...
                    <button onClick={async ()=>{ if(window.confirm("¿Realizar arqueo de caja y cerrar turno?")){ await axios.put('/api/turnos/finalizar', { turno_id: turnoActivo.id }); onUpdate(); } }} className="w-full py-10 bg-red-500 text-white font-black rounded-[45px] shadow-xl hover:bg-red-600 transition-all uppercase text-sm tracking-widest active:scale-95 shadow-red-900/40 tracking-[4px]">Cerrar Turno Corporativo</button>
                </div>
            ) : (
                <div className="space-y-10 animate-fade-in">
...
function VentasTPV({ user, turnoActivo }) {
    ...
    if(!turnoActivo) return (
        <div className="h-[650px] flex flex-col items-center justify-center bg-white rounded-[95px] border-8 border-dashed border-slate-50 opacity-20 animate-pulse italic">
            <ShoppingCart size={150} className="mb-8 opacity-10"/>
            <h2 className="text-6xl font-black uppercase tracking-tighter leading-none mb-4">Caja Inactiva</h2>
            <p className="font-bold text-xl uppercase tracking-widest">Inicie un turno administrativo</p>
        </div>
    );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 h-[800px] animate-fade-in">
            <div className="lg:col-span-2 bg-white rounded-[70px] shadow-sm border border-slate-100 p-12 flex flex-col">
                <div className="flex items-center bg-slate-100 p-10 rounded-[50px] focus-within:ring-8 ring-blue-50 transition-all shadow-inner border border-slate-200">
                    ...
                    <input className="bg-transparent border-none outline-none font-black text-3xl w-full text-slate-800 placeholder:text-slate-300 italic tracking-tighter" placeholder="ESCANEA O BUSCA PRODUCTO..." value={search} onChange={e=>setSearch(e.target.value)} autoFocus/>
                </div>
                <div className="flex-1 mt-12 overflow-y-auto grid grid-cols-2 md:grid-cols-3 gap-6">
                    {prods.filter(p=>p.nombre.toLowerCase().includes(search.toLowerCase())).map(p=>(
                        <div key={p.id} onClick={()=>{const ex=cart.find(x=>x.id===p.id); if(ex)setCart(cart.map(x=>x.id===p.id?{...x,cant:x.cant+1}:x)); else setCart([...cart,{...p,cant:1}])}} className="p-8 bg-slate-50 rounded-[45px] border border-slate-100 hover:border-blue-500 hover:scale-105 cursor-pointer transition-all flex flex-col justify-between group shadow-sm active:scale-95 h-fit">
                    ...
                    <div className="space-y-8">
                        {cart.map(x=>(
                            <div key={x.id} className="flex justify-between items-center font-bold border-b border-slate-800 pb-8 animate-slide-up">
                                <div className="flex flex-col"><span className="text-xl uppercase italic leading-none mb-3 tracking-tighter leading-none">{x.nombre.substring(0,18)}</span><span className="text-blue-400 text-[10px] font-black uppercase mt-2 tracking-widest border border-blue-400/30 px-3 py-1 rounded-full w-fit">Cant: {x.cant}</span></div>
...
function ContabilidadView({ user }) {
    ...
    return (
        <div className="bg-white p-16 rounded-[85px] shadow-sm border border-slate-100 overflow-hidden pr-2 animate-fade-in relative">
            ...
            <h3 className="font-black text-4xl mb-12 italic text-blue-600 uppercase tracking-tighter flex items-center gap-5 underline decoration-blue-100 decoration-8 underline-offset-[12px] relative z-10"><Receipt size={48}/> Libro Diario de Ingresos</h3>
            <div className="overflow-x-auto relative z-10">
                <table className="w-full text-left min-w-[1000px]">
                    <thead className="bg-slate-50 text-[12px] font-black uppercase border-b tracking-[6px] text-slate-400"><tr><th className="p-10">Fecha / Hora</th><th>Responsable de Caja</th><th>Identificador Operación</th><th className="text-right p-10">Balance Neto</th></tr></thead>
                    <tbody>{datos.map(d=>(<tr key={d.id} className="border-b hover:bg-slate-50 transition-all">
                        ...
                        <td className="font-black text-slate-800 text-xl tracking-tighter italic leading-none uppercase">Ticket TPV #{d.id.toString().padStart(6, '0')}</td>
...
function LoginScreen({ onLogin, onBuy }) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#0f172a] p-4 relative overflow-hidden">
      ...
      <div className="bg-white p-20 rounded-[80px] shadow-2xl w-full max-w-lg border-t-[15px] border-slate-900 animate-slide-up relative z-10">
        ...
        <p className="text-center text-[10px] font-black text-slate-400 uppercase tracking-[12px] mb-20 italic opacity-60 leading-none">Enterprise Management System</p>
        <form onSubmit={handleAuth} className="space-y-8">
          <input className="w-full p-8 bg-slate-100 rounded-[40px] font-black outline-none focus:ring-8 ring-blue-50 transition-all text-xl tracking-tighter" placeholder="Email Corp." value={email} onChange={e=>setEmail(e.target.value)} required />
          <input type="password" class="w-full p-8 bg-slate-100 rounded-[40px] font-black outline-none focus:ring-8 ring-blue-50 transition-all text-xl tracking-tighter" placeholder="Password" value={password} onChange={e=>setPassword(e.target.value)} required />
          <button className="w-full bg-slate-900 text-white font-black py-8 rounded-[40px] shadow-2xl hover:bg-blue-600 transition-all active:scale-95 uppercase text-xs tracking-[8px] mt-10 shadow-blue-900/30">ENTRAR AL ECOSISTEMA</button>
        </form>
        <div className="mt-16 p-8 bg-green-50 border-2 border-green-200 rounded-[45px] text-center cursor-pointer group" onClick={onBuy}>
            <p className="text-[12px] font-black text-green-700 uppercase mb-4 tracking-tighter leading-tight italic">¡Haz parte del mejor sistema empresarial!</p>
            <button className="w-full py-4 bg-green-600 text-white font-black rounded-[25px] text-xs shadow-lg group-hover:scale-105 transition-all uppercase tracking-widest flex items-center justify-center gap-3">
...
function ResumenView({ user }) {
  ...
  return <div className="space-y-16 animate-fade-in"><div className="grid grid-cols-2 md:grid-cols-4 gap-12">
          ...
          <CardStat title="Stock Crítico" value={data.lowStock} color="red" icon={<AlertTriangle size={40}/>}/>
