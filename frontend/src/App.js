    ...
    <div className={darkMode ? 'dark' : ''}>
      <div className="flex h-screen bg-slate-50 dark:bg-slate-950 font-sans overflow-hidden transition-all duration-300">
        ...
        <aside className="w-80 bg-white dark:bg-slate-900 border-r dark:border-slate-800 flex flex-col p-8 shadow-2xl z-20">
          ...
          <div className="pt-6 border-t dark:border-slate-800 mt-6 flex flex-col gap-4 text-xs font-black">
              <div className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-2xl border dark:border-slate-700 overflow-hidden">
                  ...
                  <div className="truncate">
                      ...
                      <p className="text-slate-400 uppercase font-bold">{user.cargo}</p>
                  </div>
              </div>
              <button onClick={()=>{localStorage.clear(); window.location.reload();}} className="text-red-500 uppercase p-4 rounded-2xl w-full transition hover:bg-red-50 dark:hover:bg-red-900/20">Cerrar Sesión</button>
          </div>
        </aside>
    ...
    return (
        <div className="space-y-12 animate-fade-in">
            ...
            <div className="grid grid-cols-2 gap-10">
                ...
                <div className="bg-white dark:bg-slate-900 p-10 rounded-[50px] shadow-sm border dark:border-slate-800 overflow-auto h-[400px]">
        ...
        <div className="grid grid-cols-3 gap-10 h-full animate-fade-in">
            <div className="col-span-2 space-y-6">
                <div className="bg-white dark:bg-slate-900 p-6 rounded-[30px] border dark:border-slate-800 shadow-sm flex items-center gap-4">
                    ...
                    <input className="w-full bg-transparent font-bold outline-none dark:text-white" placeholder="Buscar producto..." onChange={e=>setSearch(e.target.value)} />
                </div>
                <div className="grid grid-cols-3 gap-6 overflow-auto max-h-[600px] pr-2">
                    {prods.filter(p=>p.nombre.toLowerCase().includes(search.toLowerCase())).map(p => (
                        <div key={p.id} onClick={()=>setCart([...cart, {...p, cantidad:1}])} className="bg-white dark:bg-slate-900 p-6 rounded-[40px] border dark:border-slate-800 shadow-sm hover:border-blue-600 cursor-pointer transition-all">
            ...
            <div className="bg-white dark:bg-slate-900 p-10 rounded-[60px] shadow-2xl border-t-[20px] border-blue-600 h-fit sticky top-0 border dark:border-slate-800">
                <h3 className="text-[10px] font-black uppercase text-slate-400 mb-8 text-center tracking-widest">Facturación</h3>
                ...
                <h2 className="text-5xl font-black text-slate-800 dark:text-white text-center mb-10">{fmt(cart.reduce((s,i)=>s+(i.costo_venta*i.cantidad),0))}</h2>
                <button onClick={async()=>{ await axios.post('/api/ventas', {productos:cart, turno_id:turno.id}); alert("Venta Exitosa"); setCart([]); load(); }} className="w-full py-7 bg-blue-600 text-white font-black rounded-3xl shadow-xl uppercase">Cobrar</button>
            </div>
        </div>
    );
}
...
function InventarioView() {
    ...
    return (
        <div className="space-y-10 animate-fade-in">
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody>{items.map(p => (
                            <tr key={p.id} className="border-b dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 transition">
                                ...
                                <td className="dark:text-white">{p.stock}</td>
            ...
            {sub==='new' && (
                <form onSubmit={async(e)=>{e.preventDefault(); await axios.post('/api/productos', f); alert("Guardado"); load(); setSub('list');}} className="bg-white dark:bg-slate-900 p-12 rounded-[60px] border dark:border-slate-800 shadow-2xl max-w-2xl mx-auto space-y-6">
                    <h3 className="text-2xl font-black italic dark:text-white text-center">Nuevo Ingreso</h3>
                    <input className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" placeholder="Nombre Comercial" value={f.nombre} onChange={e=>setF({...f, nombre:e.target.value})} required/>
                    <input className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" placeholder="Código SKU" value={f.sku} onChange={e=>setF({...f, sku:e.target.value})} required/>
                    <div className="grid grid-cols-2 gap-6">
                        <input className="p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" placeholder="Costo $" type="number" onChange={e=>setF({...f, costo_compra:e.target.value})} required/>
                        <input className="p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" placeholder="Venta $" type="number" onChange={e=>setF({...f, costo_venta:e.target.value})} required/>
                    </div>
                    <div className="grid grid-cols-2 gap-6">
                        <input className="p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" placeholder="Stock" type="number" onChange={e=>setF({...f, stock:e.target.value})} required/>
                        <input className="p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" type="date" onChange={e=>setF({...f, fecha_vencimiento:e.target.value})} required/>
                    </div>
                    <select className="p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-black border-none dark:text-white" onChange={e=>setF({...f, bodega_id:e.target.value})}>
            ...
            {sub==='bod' && (
                <div className="grid grid-cols-2 gap-12 h-fit">
                    <div className="bg-white dark:bg-slate-900 p-12 rounded-[50px] border dark:border-slate-800 shadow-2xl space-y-6 h-fit text-center">
                        ...
                        <textarea className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl border-none mb-6 dark:text-white" id="bD" placeholder="Ubicación/Detalles" />
                    ...
                    <div className="grid grid-cols-2 gap-10">
                        {recetas.map(r => (
                            <div key={r.id} className="p-10 bg-slate-50 dark:bg-slate-800 rounded-[45px] border-2 border-dashed border-slate-200 dark:border-slate-700">
                                ...
                                <div className="text-xs font-bold text-slate-500 italic">Insumos vinculados</div>
                            </div>
                        ))}
                        <button onClick={async()=>{ const n = prompt("Nombre?"); await axios.post('/api/produccion/recetas', {nombre:n, descripcion:'...'}); load(); }} className="p-10 border-4 border-dotted rounded-[45px] flex flex-col items-center justify-center text-slate-300 font-black hover:bg-slate-50 dark:hover:bg-slate-800 transition-all uppercase">+ Nueva Receta</button>
                    </div>
                </div>
            )}
            ...
            {sub==='ordenes' && (
                <div className="space-y-10">
                    <div className="bg-white dark:bg-slate-900 p-10 rounded-[50px] border dark:border-slate-800 shadow-2xl flex gap-8 items-end border-t-[15px] border-blue-600">
                        <div className="flex-1 space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 ml-4">Kit</label><select id="rI" className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-black border-none dark:text-white">{recetas.map(r=><option key={r.id} value={r.id}>{r.nombre_kit}</option>)}</select></div>
                    ...
                    {ordenes.map(o => (
                        <div key={o.id} className="bg-white dark:bg-slate-900 p-10 rounded-[45px] border shadow-sm flex justify-between items-center group hover:border-blue-500 transition-all">
                            <div><h4 className="text-2xl font-black text-slate-900 dark:text-white italic">{o.numero_orden} - {o.nombre_kit}</h4><p className="text-[10px] font-bold text-slate-400 mt-3">Montó: {o.usuario_monta}</p></div>
                            <div className="flex items-center gap-8">
                                ...
                                {o.estado==='Alistada' && <button onClick={async()=>{ const out = prompt("Salida?"); await axios.put(`/api/produccion/ordenes/${o.id}/avanzar`, {estado:'Procesada', out, obs:'...'}); load(); }} className="bg-orange-500 text-white px-8 py-3 rounded-2xl font-black text-[10px] uppercase">Procesar</button>}
                                {o.estado==='Procesada' && <button onClick={async()=>{ await axios.put(`/api/produccion/ordenes/${o.id}/avanzar`, {estado:'Terminada'}); load(); }} className="bg-green-600 text-white px-8 py-3 rounded-2xl font-black text-[10px] uppercase">Finalizar</button>}
                            </div>
                        </div>
                    ))}
                </div>
...
function NominaView() {
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 h-fit animate-fade-in">
            <form onSubmit={async(e)=>{e.preventDefault(); await axios.post('/api/empleados', f); alert("Vinculado"); load();}} className="bg-white dark:bg-slate-900 p-12 rounded-[50px] border dark:border-slate-800 shadow-2xl space-y-6 h-fit">
                <h3 className="text-2xl font-black italic dark:text-white">Vincular Funcionario</h3>
                <input className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl font-bold border-none dark:text-white" placeholder="Nombre" value={f.nombre} onChange={e=>setF({...f, nombre:e.target.value})}/>
                <input className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl font-bold border-none dark:text-white" placeholder="Doc" value={f.documento} onChange={e=>setF({...f, documento:e.target.value})}/>
                ...
                {emps.map(e => (
                    <div key={e.id} className="bg-white dark:bg-slate-900 p-10 rounded-[45px] border dark:border-slate-800 shadow-sm group hover:border-blue-600 transition-all flex flex-col gap-6">
                        ...
                        <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-800 p-6 rounded-[30px]">
                            ...
                            <button onClick={()=>alert(`Liquidando...\nSubsidio: $162.000\nTotal: ${fmt(e.valor_dia*30 + 162000)}`)} className="bg-slate-900 dark:bg-blue-600 text-white px-8 py-3 rounded-2xl font-black text-[10px] uppercase shadow-lg">LIQUIDAR</button>
                        </div>
                    </div>
                ))}
            </div>
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 animate-fade-in">
            <div className="bg-white dark:bg-slate-900 p-12 rounded-[60px] border dark:border-slate-800 shadow-2xl space-y-10">
                <div className="flex items-center gap-8 border-b dark:border-slate-800 pb-10 text-center">
                    <div className="w-32 h-32 bg-slate-100 dark:bg-slate-800 rounded-[45px] flex items-center justify-center text-slate-300 overflow-hidden border-4 border-white dark:border-slate-900 shadow-xl mx-auto">
                        {f.avatar_url ? <img src={f.avatar_url} className="w-full h-full object-cover" /> : <User size={48}/>}
                    </div>
                    <div className="text-left">
                        <h3 className="text-3xl font-black text-slate-900 dark:text-white mb-2">Mi Perfil</h3>
                        <p className="text-slate-400 font-bold uppercase text-[10px]">Cloud Premium 2026</p>
                    </div>
                </div>
                <form onSubmit={async(e)=>{e.preventDefault(); await axios.put('/api/perfil', f); alert("Perfil Actualizado"); setUser({...user, ...f}); localStorage.setItem('erp_user', JSON.stringify({...user, ...f}));}} className="space-y-6">
                    <input className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white outline-none" value={f.nombre} placeholder="Nombre" onChange={e=>setF({...f, nombre:e.target.value})} />
                    <input className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white outline-none" value={f.avatar_url} placeholder="Foto Link (https://...)" onChange={e=>setF({...f, avatar_url:e.target.value})} />
                    <input className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white outline-none" value={f.telefono} placeholder="Teléfono" onChange={e=>setF({...f, telefono:e.target.value})} />
            ...
            <div className="bg-blue-600 p-12 rounded-[60px] text-white shadow-2xl h-fit">
                <h3 className="text-2xl font-black italic mb-4">Soporte Técnico VIP</h3>
                <p className="text-blue-100 font-bold text-sm leading-relaxed mb-10">Línea prioritaria para resolver cualquier duda técnica sobre el ERP.</p>
                <button onClick={()=>window.open('https://wa.me/573158022191')} className="bg-white text-blue-600 px-8 py-4 rounded-3xl font-black uppercase text-xs">Soporte WhatsApp</button>
            </div>
        </div>
    );
}
            ...
            <div className="bg-white dark:bg-slate-900 p-20 rounded-[60px] border dark:border-slate-800 shadow-2xl border-t-[20px] border-blue-600">
                ...
                {activo ? (
                    <div className="space-y-8 text-left">
                        <h3 className="text-3xl font-black italic dark:text-white text-center">Caja Abierta</h3>
                        <div className="grid grid-cols-2 gap-6">
                            <div className="bg-slate-50 dark:bg-slate-800 p-8 rounded-[40px]"><p className="text-[10px] font-black text-slate-400 mb-2">Base</p><p className="text-3xl font-black dark:text-white">{fmt(activo.base_caja)}</p></div>
                            <div className="bg-blue-50 dark:bg-blue-900/20 p-8 rounded-[40px] border border-blue-100 dark:border-blue-900"><p className="text-[10px] font-black text-blue-400 mb-2">Vendido</p><p className="text-3xl font-black text-blue-600">{fmt(activo.total_vendido)}</p></div>
                        </div>
                        <button onClick={async()=>{ if(confirm("Cerrar?")){ await axios.put('/api/turnos/finalizar', {turno_id: activo.id}); load(); }}} className="w-full py-7 bg-red-500 text-white font-black rounded-3xl shadow-xl uppercase">Finalizar Turno</button>
                    </div>
                ) : (
