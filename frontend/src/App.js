...
import * as XLSX from 'xlsx'; // <--- DEBE ESTAR AQUÍ ARRIBA
import { 
...
const imprimirFactura = (venta, cliente) => {
    try {
        ...
        // --- CAMBIO CLAVE AQUÍ: Llamamos a la función autoTable directamente ---
        autoTable(doc, {
            startY: 40,
      ...
      <aside className="w-72 bg-white border-r flex flex-col z-20 shadow-xl px-6">
        ...
        <div className="py-8 border-t space-y-4">
            <div className="bg-slate-50 p-4 rounded-3xl flex items-center gap-3">
                ...
                <div className="overflow-hidden"><p className="font-black text-slate-800 text-sm truncate">{user.nombre}</p><p className="text-[10px] font-bold text-slate-400">ADMIN</p></div>
            </div>
...
function VentasView({ user, turnoActivo }) {
  ...
  // --- LÓGICA DE PISTOLA DE CÓDIGO DE BARRAS ---
  useEffect(() => {
    let barcode = "";
    const handleKey = (e) => {
        if(e.key === 'Enter') {
            ...
            if(prod) {
                setVentaForm({ producto_id: prod.id, nombre: prod.nombre, precio: prod.precio, cantidad: 1 });
            }
            barcode = "";
        } else {
            barcode += e.key;
  ...
  const procesar = async () => {
      ...
      if(metodo === 'Efectivo' && (parseFloat(pagaCon) < totalVenta || !pagaCon)) {
          return window.alert("El monto pagado es insuficiente.");
      }
      ...
      try {
          const res = await axios.post('/ventas', {
              ...
              metodo_pago: metodo,
              es_electronica: esElectronica, // Aquí enviamos el estado al backend
              cliente: esElectronica ? cliente : { nombre: 'General', nit: '222' }, // Corregido: esElectronica
              pago_recibido: pagaCon || totalVenta,
              cambio: devuelta > 0 ? devuelta : 0
          });

          if(res.data.success) {
              // PASO 1: Generar Recibo
              imprimirFactura({ 
                ...ventaForm, 
                total: totalVenta, 
                responsable: user.nombre, 
                ...
                cambio: devuelta 
              }, esElectronica ? cliente : null);

              // PASO 2: Limpiar Interfaz
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 animate-fade-in">
            ...
            <div className="space-y-6">
                ...
                {ventaForm.nombre && (
                    <div className="bg-blue-50 p-6 rounded-3xl flex justify-between items-center border border-blue-100">
                        <span className="font-black text-blue-900">{ventaForm.nombre}</span>
                        <input type="number" className="w-20 p-2 rounded-xl text-center font-bold" value={ventaForm.cantidad} onChange={e=>setVentaForm({...ventaForm, cantidad: e.target.value})} />
                    </div>
                )}
                ...
                {metodo === 'Efectivo' && (
                    <div className="bg-slate-50 p-6 rounded-3xl border-2 border-dashed border-slate-200 animate-slide-up">
                        <div className="flex justify-between items-center mb-2">
                            <span className="font-black text-slate-400 text-xs">RECIBIDO:</span>
                            <input type="number" className="w-40 p-3 rounded-xl text-right font-black text-2xl text-green-600 outline-none" placeholder="$ 0" value={pagaCon} onChange={e=>setPagaCon(e.target.value)} />
                        </div>
                        <div className="flex justify-between font-black text-blue-600 border-t pt-2">
                            <span>CAMBIO:</span>
                            <span className="text-xl">{fmt(devuelta)}</span>
                        </div>
                    </div>
                )}
                ...
                {esElectronica && (
                    <div className="grid grid-cols-2 gap-4 animate-fade-in">
                        <input className="p-3 bg-slate-100 border-none rounded-xl text-xs font-bold" placeholder="Nombre del Cliente" onChange={e=>setCliente({...cliente, nombre: e.target.value})} />
                        <input className="p-3 bg-slate-100 border-none rounded-xl text-xs font-bold" placeholder="NIT / Cédula" onChange={e=>setCliente({...cliente, nit: e.target.value})} />
                        <input className="p-3 bg-slate-100 border-none rounded-xl text-xs font-bold" placeholder="Email destino" onChange={e=>setCliente({...cliente, email: e.target.value})} />
                        <input className="p-3 bg-slate-100 border-none rounded-xl text-xs font-bold" placeholder="Teléfono" onChange={e=>setCliente({...cliente, tel: e.target.value})} />
                    </div>
                )}
            </div>
        </div>
        ...
        <div className="bg-white p-12 rounded-[40px] shadow-2xl border border-slate-100 text-center flex flex-col justify-between">
            <div>
                <p className="text-xs font-black text-slate-300 uppercase tracking-widest mb-4">Total a Pagar</p>
...
function InventarioView({ user }) {
  ...
  const handleImportExcel = (e) => {
    ...
    reader.onload = async (evt) => {
      ...
      // Mapear los datos para que coincidan con la base de datos
      // El Excel debe tener columnas: Nombre, SKU, Precio, Stock
      ...
      if (window.confirm(`¿Deseas importar ${productosImportados.length} productos desde el archivo?`)) {
        try {
          await axios.post('/productos/importar', { 
            productos: productosImportados, 
            responsable: user.nombre 
          });
          window.alert("✅ Importación masiva exitosa");
          load();
        } catch (error) {
          window.alert("❌ Error al importar. Revisa el formato del archivo.");
        }
      }
    };
  ...
  const handleCrear = async (e) => {
    e.preventDefault();
    await axios.post('/productos', {...form, responsable: user.nombre});
    window.alert("¡Producto registrado!");
    load();
    setForm({ nombre: '', sku: '', precio: '', stock: '', min_stock: 5 });
  };
    ...
    <div className="space-y-10 animate-fade-in">
        <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100">
            <div className="flex justify-between items-center mb-8">
                <h3 className="font-black text-xl tracking-tighter">Gestión de Inventario</h3>
                
                {/* BOTÓN DE CARGA MASIVA */}
                <label className="bg-slate-900 text-white px-6 py-3 rounded-2xl font-black text-xs cursor-pointer hover:bg-black transition-all flex items-center gap-2 shadow-xl shadow-slate-200">
            ...
            <form onSubmit={handleCrear} className="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <button className="bg-blue-600 text-white font-black rounded-2xl p-4 shadow-xl shadow-blue-100 hover:scale-105 transition-all">GUARDAR</button>
            </form>
        </div>
                ...
                <tbody>
                    {productos.map(p=>(
                        <tr key={p.id} className="border-b hover:bg-slate-50 transition">
                            ...
                            <td className="font-black text-slate-800">{p.stock}</td>
                            <td className="p-8 text-center">
                                {p.stock <= p.min_stock ? 
                                    <span className="bg-red-50 text-red-500 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest">Agotándose</span> : 
                                    <span className="bg-green-50 text-green-600 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest">Disponible</span>
                                }
                            </td>
                        </tr>
                    ))}
...
function NominaView({ user }) {
  ...
  const bancosCO = ["Bancolombia", "Nequi", "Daviplata", "Davienda", "Lulo Bank", "Nu Bank"];
  ...
  const pagar = async () => {
      if(window.confirm(`¿Confirmar pago a ${preview.nombre}?`)) {
          await axios.post('/nomina/liquidar', { ...formLiq, extras: formLiq.extras, responsable: user.nombre });
          window.alert("Nómina Pagada y Comprobante enviado al correo.");
          load(); setMode('history'); setPreview(null);
      }
  };
  ...
  return (
    <div className="space-y-10 animate-fade-in">
      ...
      {mode === 'empleados' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100">
                ...
                <form onSubmit={async (e)=>{e.preventDefault(); await axios.post('/empleados', formEmp); load(); setFormEmp({nombre:'',email:'',salario:'',eps:'',arl:'',pension:''});}} className="space-y-4">
                    ...
                    <input className="w-full p-4 bg-slate-50 border-none rounded-2xl font-bold" type="number" placeholder="Salario Base" value={formEmp.salario} onChange={e=>setFormEmp({...formEmp, salario: e.target.value})} required/>
                    <div className="grid grid-cols-3 gap-2">
                        <input className="p-3 bg-slate-50 border-none rounded-xl text-[10px] font-black uppercase" placeholder="EPS" value={formEmp.eps} onChange={e=>setFormEmp({...formEmp, eps: e.target.value})} required/>
                        <input className="p-3 bg-slate-50 border-none rounded-xl text-[10px] font-black uppercase" placeholder="ARL" value={formEmp.arl} onChange={e=>setFormEmp({...formEmp, arl: e.target.value})} required/>
                        <input className="p-3 bg-slate-50 border-none rounded-xl text-[10px] font-black uppercase" placeholder="F.P" value={formEmp.pension} onChange={e=>setFormEmp({...formEmp, pension: e.target.value})} required/>
                    </div>
                    <button className="w-full bg-blue-600 text-white font-black py-4 rounded-2xl shadow-xl shadow-blue-100">VINCULAR</button>
                </form>
            </div>
            <div className="lg:col-span-2 grid grid-cols-2 gap-4 h-fit">
                {empleados.map(e=>(
                    <div key={e.id} className="bg-white p-6 rounded-[32px] shadow-sm border border-slate-100 flex items-center gap-6">
                        <div className="w-16 h-16 bg-slate-50 rounded-2xl flex items-center justify-center font-black text-2xl text-blue-600">{e.nombre.charAt(0)}</div>
      ...
      {mode === 'liquidar' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
              <div className="bg-white p-12 rounded-[40px] shadow-xl border border-green-100">
                  <h3 className="font-black text-2xl mb-8 text-green-800 tracking-tighter flex items-center gap-3"><Calculator/> LIQUIDADOR 2026</h3>
                  <div className="space-y-6">
                      <div><label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-4 mb-2 block">Empleado</label>
                  ...
                  {preview ? (
                      <div className="space-y-6 animate-fade-in">
                          ...
                          <div className="space-y-3 font-bold text-sm text-slate-600">
                              ...
                              <div className="flex justify-between text-red-500 border-t pt-4"><span>Deducciones Ley:</span><span>-{fmt(preview.salud + preview.pension)}</span></div>
                          </div>
                          ...
                          <button onClick={pagar} className="w-full bg-slate-900 text-white font-black py-6 rounded-[32px] shadow-xl hover:scale-[1.02] transition-all">CONFIRMAR Y ENVIAR EMAIL</button>
                      </div>
                  ) : <div className="h-full flex items-center justify-center opacity-20"><Mail size={100}/></div>}
              </div>
          </div>
      )}
      ...
      {mode === 'history' && (
          <div className="bg-white rounded-[40px] shadow-sm overflow-hidden border border-slate-100">
            <table className="w-full text-left text-sm"><thead className="bg-slate-50/50"><tr className="text-[10px] font-black uppercase text-slate-400 border-b"><th className="p-8">Fecha Pago</th><th>Empleado</th><th>Devengado</th><th>Salud/Pens</th><th className="p-8 text-right">Neto Pagado</th></tr></thead><tbody>{nominas.map(n => (<tr key={n.id} className="border-b hover:bg-slate-50"><td className="p-8 text-xs font-black text-slate-500">{new Date(n.fecha_pago).toLocaleDateString()}</td><td className="font-black text-slate-800 text-lg tracking-tight">{n.nombre_empleado}</td><td className="font-bold text-slate-600">{fmt(n.total_devengado)}</td><td className="font-bold text-red-500">-{fmt(n.total_deducido)}</td><td className="p-8 font-black text-green-600 text-xl text-right">{fmt(n.neto_pagar)}</td></tr>))}</tbody></table>
          </div>
      )}
    </div>
  );
            ...
            {subTab === 'diario' && (
                <div className="space-y-6">
                    {Object.values(gruposDiario).map(comp => (
                        <div key={comp.id} className="bg-white rounded-[40px] shadow-sm border overflow-hidden animate-slide-up"><div className="px-8 py-4 bg-slate-50 border-b flex justify-between items-center"><span className="font-black text-slate-800 tracking-tight">{comp.tipo} #{comp.id}</span><span className="text-xs font-black text-slate-400">{new Date(comp.fecha).toLocaleDateString()}</span></div><table className="w-full text-left text-sm"><tbody>{comp.asientos.map((as, idx) => (<tr key={idx} className="border-b last:border-none hover:bg-slate-50 transition"><td className="p-4 pl-12 font-bold text-slate-600"><span className="text-blue-600 mr-2">{as.cuenta_codigo}</span> {as.cuenta_nombre}</td><td className="text-right font-black text-slate-800">{as.debito > 0 ? fmt(as.debito) : '-'}</td><td className="p-4 pr-12 text-right font-black text-slate-800">{as.credito > 0 ? fmt(as.credito) : '-'}</td></tr>))}</tbody></table></div>
                    ))}
                </div>
            )}
            {subTab === 'balance' && (
                <div className="bg-white rounded-[40px] shadow-sm border overflow-hidden"><table className="w-full text-left"><thead className="bg-slate-900 text-white"><tr className="text-[10px] font-black uppercase tracking-widest"><th className="p-8">Código</th><th>Cuenta Contable</th><th className="text-right">Mov. Débito</th><th className="text-right">Mov. Crédito</th><th className="p-8 text-right">Saldo Final</th></tr></thead><tbody>{balance.map((cta, i) => (<tr key={i} className="border-b hover:bg-blue-50 transition group"><td className="p-6 font-black text-blue-600">{cta.codigo}</td><td className="font-bold text-slate-700">{cta.nombre}</td><td className="text-right font-medium text-slate-400">{fmt(cta.total_debito)}</td><td className="text-right font-medium text-slate-400">{fmt(cta.total_credito)}</td><td className={`p-8 text-right font-black ${cta.saldo < 0 ? 'text-red-500' : 'text-slate-900'}`}>{fmt(Math.abs(cta.saldo))} {cta.saldo < 0 ? '(Cr)' : '(Db)'}</td></tr>))}</tbody></table></div>
            )}
        </div>
    );
}
