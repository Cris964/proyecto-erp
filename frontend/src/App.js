...
import { 
  ...
  ScanBarcode, Upload, X, ShieldCheck, ChevronDown, UserCircle, RefreshCcw, Menu
} from 'lucide-react';
    ...
    <div className="flex h-screen overflow-hidden bg-slate-50 flex-col md:flex-row">
      ...
      <main className="flex-1 overflow-auto p-4 md:p-10">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-10 gap-4">
            ...
            {turnoActivo ? <div className="w-full md:w-auto px-4 py-2 bg-green-100 text-green-700 rounded-xl text-[10px] font-black flex items-center justify-center gap-2 border border-green-200 uppercase tracking-widest"><div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div> EN TURNO: {user.nombre.toUpperCase()}</div> : <div className="w-full md:w-auto px-4 py-2 bg-red-100 text-red-700 rounded-xl text-[10px] font-black border border-red-200 text-center uppercase tracking-widest">Caja Cerrada</div>}
        </header>
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
          <div className="bg-white p-6 md:p-10 rounded-[30px] md:rounded-[40px] shadow-sm border border-slate-100 h-80 md:h-96">
              <h3 className="font-black text-slate-800 mb-6 tracking-tight text-lg uppercase italic">Ventas Semanales</h3>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in">
        <div className="lg:col-span-2 space-y-6">
            <div className="bg-white p-6 rounded-[30px] shadow-sm border border-blue-50">
              <div className="flex items-center bg-slate-50 p-4 rounded-2xl border focus-within:ring-2 ring-blue-500 transition-all">
                ...
                <input autoFocus className="bg-transparent border-none outline-none w-full font-bold" placeholder="Pasa la pistola..." value={searchTerm} onChange={e=>setSearchTerm(e.target.value)} />
              </div>
        ...
        <div className="bg-white p-8 rounded-[30px] md:rounded-[40px] shadow-xl border flex flex-col justify-between h-fit sticky top-10">
            ...
            <button onClick={procesar} className="w-full bg-blue-600 text-white font-black py-5 rounded-[25px] shadow-xl hover:scale-105 transition-all active:scale-95 text-lg mt-6">CONFIRMAR VENTA</button>
        </div>
    </div>
  );
}
...
function NominaView({ user }) {
  ...
  const load = useCallback(() => { axios.get('/empleados').then(res => setEmpleados(Array.isArray(res.data) ? res.data : [])); axios.get('/nomina/historial').then(res => setNominas(Array.isArray(res.data) ? res.data : [])); }, []);
  useEffect(() => { load(); }, [load]);
  const verPerfil = async (emp) => { setSelectedEmp(emp); const res = await axios.get(`/empleados/${emp.id}/historial`); setEmpHistory(res.data); setMode('perfil'); };
  const calcular = () => {
    ...
    <div className="space-y-10 animate-fade-in">
      ...
      {mode === 'liquidar' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
              ...
              <div className="bg-white p-12 rounded-[40px] shadow-2xl border-l-[12px] border-blue-600 flex flex-col justify-between">{preview ? (<div className="space-y-6 animate-fade-in"><div className="text-center border-b pb-8"><h4 className="text-3xl font-black text-slate-800 tracking-tighter">{preview.nombre}</h4></div><div className="bg-blue-600 p-8 rounded-[32px] text-center text-5xl font-black text-white shadow-xl">{fmt(preview.neto)}</div><button onClick={async ()=>{if(window.confirm(`¿Confirmar pago?`)){await axios.post('/api/nomina/liquidar', {...formLiq, extras: formLiq.extras, responsable: user.nombre}); window.alert("Éxito"); load(); setMode('history'); setPreview(null);}}} className="w-full bg-slate-900 text-white font-black py-6 rounded-[32px] shadow-xl hover:scale-102 transition-all">PAGAR</button></div>) : <div className="h-full flex items-center justify-center opacity-20 flex-col"><Mail size={100}/><p className="font-black mt-4 uppercase tracking-widest">Listo</p></div>}</div>
          </div>
      )}
      {mode === 'empleados' && (
...
function AdminView() {
    ...
    useEffect(() => { load(); }, []);
    const handleDelete = async (id) => { if(window.confirm("¿Eliminar?")) { await axios.delete(`/api/admin/usuarios/${id}`); load(); } };
    return (
        <div className="space-y-10 animate-fade-in">
            <div className="bg-white p-10 rounded-[40px] shadow-sm border border-slate-100 h-fit">
                ...
                <form onSubmit={async (e)=>{e.preventDefault(); await axios.post('/register', form); load(); setForm({nombre:'',email:'',password:'',cargo:'Vendedor'});}} className="grid grid-cols-1 md:grid-cols-5 gap-4">
                    ...
                    <input className="p-4 bg-slate-50 border-none rounded-2xl font-bold text-sm" type="password" placeholder="Pass" value={form.password} onChange={e=>setForm({...form, password: e.target.value})} required/>
            ...
            <div className="bg-white rounded-[40px] shadow-sm overflow-hidden border border-slate-100 pr-2"><div className="overflow-x-auto"><table className="w-full text-left min-w-[500px]"><thead className="bg-slate-50/50 text-[10px] font-black uppercase tracking-widest border-b"><tr><th className="p-8">Nombre</th><th>Email</th><th>Rol</th><th className="p-8 text-center">Acción</th></tr></thead><tbody>{usuarios.map(u => (<tr key={u.id} className="border-b hover:bg-slate-50 transition"><td className="p-8 font-black">{u.nombre}</td><td>{u.email}</td><td><span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-[10px] font-black uppercase">{u.cargo}</span></td><td className="p-8 text-center"><button onClick={()=>handleDelete(u.id)} className="text-red-500 font-bold hover:bg-red-50 p-2 rounded-xl">Eliminar</button></td></tr>))}</tbody></table></div></div>
        </div>
    );
}
