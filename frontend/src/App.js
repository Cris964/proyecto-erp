      ...
      <div className="flex h-screen bg-slate-50 dark:bg-slate-950 font-sans overflow-hidden transition-colors duration-300">
        ...
        <aside className="w-80 bg-white dark:bg-slate-900 border-r dark:border-slate-800 flex flex-col p-8 shadow-2xl z-20">
          <div className="mb-10 flex items-center justify-between">
            <h1 className="text-3xl font-black italic tracking-tighter text-slate-900 dark:text-white">ACCUCLOUD<span className="text-blue-600">.</span></h1>
            <button onClick={toggleTheme} className="p-2 bg-slate-100 dark:bg-slate-800 rounded-xl text-slate-500 hover:scale-110 transition">
          ...
          <div className="pt-6 border-t dark:border-slate-800 mt-6 flex flex-col gap-4">
              <div className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-2xl border dark:border-slate-700">
                  <div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center text-white font-black overflow-hidden">
              ...
              <button onClick={()=>{localStorage.clear(); window.location.reload();}} className="text-red-500 font-black text-xs uppercase p-4 rounded-2xl w-full transition hover:bg-red-50 dark:hover:bg-red-900/20">Cerrar Sesión</button>
          </div>
        </aside>
    ...
    return (
        <div className="space-y-12">
            <div className="grid grid-cols-3 gap-8 text-white">
                ...
                <CardStat title="Suscripción Pro" value="Activa 2026" icon={<ShieldCheck/>} color="green" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-10">
                ...
                <div className="bg-white dark:bg-slate-900 p-10 rounded-[50px] shadow-sm border dark:border-slate-800 overflow-auto h-[450px]">
        ...
        <div className="grid grid-cols-3 gap-10 h-full">
            <div className="col-span-2 space-y-6">
                <div className="bg-white dark:bg-slate-900 p-6 rounded-[30px] border dark:border-slate-800 shadow-sm flex items-center gap-4">
                    ...
                    <input className="w-full bg-transparent font-bold outline-none dark:text-white" placeholder="Buscar..." onChange={e=>setSearch(e.target.value)} />
                </div>
                <div className="grid grid-cols-3 gap-6 overflow-auto max-h-[600px] pr-2">
                    {prods.filter(p=>p.nombre.toLowerCase().includes(search.toLowerCase())).map(p => (
                        <div key={p.id} onClick={()=>addToCart(p)} className="bg-white dark:bg-slate-900 p-6 rounded-[40px] border dark:border-slate-800 shadow-sm hover:border-blue-600 cursor-pointer transition-all">
            ...
            <div className="bg-white dark:bg-slate-900 p-10 rounded-[60px] shadow-2xl border-t-[20px] border-blue-600 flex flex-col justify-between h-fit sticky top-0">
                ...
                <div className="space-y-4 mb-10 max-h-60 overflow-auto">
                    {cart.map((c, i) => (
                        <div key={i} className="flex justify-between bg-slate-50 dark:bg-slate-800 p-3 rounded-2xl">
                            <span className="font-bold text-xs dark:text-white">{c.nombre} x{c.cantidad}</span>
                            <span className="font-black text-xs text-blue-600">{fmt(c.costo_venta * c.cantidad)}</span>
                        </div>
                    ))}
                </div>
...
function InventarioView() {
    ...
    const [fP, setFP] = useState({ sku:'', nombre:'', costo_compra:0, costo_venta:0, stock:0, min_stock:5, bodega_id:'' });

    const load = useCallback(() => axios.get('/productos').then(res => setItems(res.data)), []);
    useEffect(() => { load(); }, [load]);

    return (
        <div className="space-y-8">
            <div className="flex gap-4 p-2 bg-white dark:bg-slate-900 rounded-[28px] w-fit shadow-lg border dark:border-slate-800">
                <button onClick={()=>setSub('list')} className={`px-10 py-3 rounded-2xl font-black text-xs ${sub==='list'?'bg-blue-600 text-white':'text-slate-400'}`}>LISTADO</button>
                <button onClick={()=>setSub('new')} className={`px-10 py-3 rounded-2xl font-black text-xs ${sub==='new'?'bg-blue-600 text-white':'text-slate-400'}`}>AGREGAR</button>
            </div>

            {sub==='list' && (
                <div className="bg-white dark:bg-slate-900 rounded-[50px] border shadow-2xl overflow-hidden">
                    <table className="w-full text-left">
                        ...
                        <tbody>{items.map(p => (
                            <tr key={p.id} className="border-b dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 transition">
                                ...
                                <td><span className="bg-green-100 text-green-700 px-3 py-1 rounded-xl font-black">{(((p.costo_venta-p.costo_compra)/p.costo_venta)*100).toFixed(0)}%</span></td>
                            </tr>
                        ))}</tbody>
                    </table>
                </div>
            ...
            {sub==='new' && (
                <form onSubmit={async(e)=>{e.preventDefault(); await axios.post('/productos', fP); alert("Guardado"); load(); setSub('list');}} className="bg-white dark:bg-slate-900 p-12 rounded-[60px] border dark:border-slate-800 shadow-2xl max-w-2xl mx-auto space-y-6">
                    <h3 className="text-2xl font-black italic text-slate-800 dark:text-white text-center">Nuevo Ingreso a Stock</h3>
                    <input className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" placeholder="Nombre" onChange={e=>setFP({...fP, nombre:e.target.value})}/>
                    <input className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" placeholder="SKU" onChange={e=>setFP({...fP, sku:e.target.value})}/>
                    <div className="grid grid-cols-2 gap-6">
                        <input className="p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" placeholder="Costo Compra" type="number" onChange={e=>setFP({...fP, costo_compra:e.target.value})}/>
                        <input className="p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" placeholder="Costo Venta" type="number" onChange={e=>setFP({...fP, costo_venta:e.target.value})}/>
                        <input className="p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" placeholder="Stock" type="number" onChange={e=>setFP({...fP, stock:e.target.value})}/>
                        <input className="p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" type="date" onChange={e=>setFP({...fP, fecha_vencimiento:e.target.value})}/>
                    </div>
...
function NominaView() {
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 h-fit">
            <form onSubmit={async(e)=>{e.preventDefault(); await axios.post('/empleados', fN); alert("Vinculado"); load();}} className="bg-white dark:bg-slate-900 p-12 rounded-[50px] border dark:border-slate-800 shadow-2xl space-y-6 h-fit">
                ...
                <input className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl font-bold border-none dark:text-white" type="number" placeholder="Valor Día" onChange={e=>setFN({...fN, valor_dia:e.target.value})}/>
                <div className="grid grid-cols-3 gap-2">
                    <input className="p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border text-[9px] font-black dark:text-white uppercase" placeholder="EPS" onChange={e=>setFN({...fN, eps:e.target.value})}/>
                    <input className="p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border text-[9px] font-black dark:text-white uppercase" placeholder="ARL" onChange={e=>setFN({...fN, arl:e.target.value})}/>
                    <input className="p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border text-[9px] font-black dark:text-white uppercase" placeholder="Pen" onChange={e=>setFN({...fN, pension:e.target.value})}/>
                </div>
            ...
            <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-8">
                {emps.map(e => (
                    <div key={e.id} className="bg-white dark:bg-slate-900 p-10 rounded-[50px] border dark:border-slate-800 shadow-sm group hover:border-blue-600 transition-all">
                        <div className="flex justify-between items-center border-b dark:border-slate-800 pb-6 mb-6">
                        ...
                        <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-800 p-6 rounded-[30px]">
                            ...
                            <button onClick={()=>alert(`Neto Mes: ${fmt(e.valor_dia*30 + 162000)}`)} className="bg-slate-900 dark:bg-blue-600 text-white px-8 py-3 rounded-2xl font-black text-[10px] uppercase shadow-lg">LIQUIDAR</button>
                        </div>
                    </div>
                ))}
            </div>
...
function CajaView({ user }) {
    ...
    const load = useCallback(() => axios.get('/turnos/activo/'+user.id).then(res => setActivo(res.data)).catch(()=>setActivo(null)), [user.id]);
    useEffect(() => { load(); }, [load]);

    return (
        <div className="max-w-2xl mx-auto mt-20">
            <div className="bg-white dark:bg-slate-900 p-20 rounded-[60px] border shadow-2xl text-center border-t-[20px] border-blue-600">
                ...
                {activo ? (
                    <div className="space-y-8 animate-fade-in text-left">
                        <h3 className="text-3xl font-black italic dark:text-white text-center">Turno de {user.nombre}</h3>
                        <div className="grid grid-cols-2 gap-6">
                        ...
                        <button onClick={async()=>{ if(confirm("¿Cerrar?")){ await axios.put('/turnos/finalizar', {turno_id: activo.id}); load(); }}} className="w-full py-7 bg-red-500 text-white font-black rounded-3xl shadow-xl uppercase">Realizar Cierre Mayor</button>
                    </div>
                ) : (
                    <div className="space-y-8 animate-fade-in text-center">
                        ...
                        <button onClick={async()=>{ await axios.post('/turnos/iniciar', {base_caja: document.getElementById('bC').value}); load(); }} className="w-full py-7 bg-blue-600 text-white font-black rounded-3xl shadow-xl uppercase">Iniciar Turno Cloud</button>
                    </div>
                )}
            </div>
        </div>
...
function ConfigView({ user, setUser }) {
    const [fP, setFP] = useState({ nombre:user.nombre, telefono:user.telefono||'', direccion:user.direccion||'', avatar_url:user.avatar_url||'' });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 animate-fade-in">
                ...
                <div className="flex items-center gap-8 border-b dark:border-slate-800 pb-10">
                    <div className="relative group">
                        <div className="w-32 h-32 bg-slate-100 dark:bg-slate-800 rounded-[45px] flex items-center justify-center text-slate-300 overflow-hidden border-4 border-white dark:border-slate-900 shadow-xl">
                           {fP.avatar_url ? <img src={fP.avatar_url} className="w-full h-full object-cover" /> : <User size={48}/>}
                        </div>
                    ...
                    <div><h3 className="text-3xl font-black text-slate-900 dark:text-white leading-none mb-2">Mi Perfil</h3><p className="text-slate-400 font-bold uppercase text-xs tracking-widest">SaaS Cloud Premium</p></div>
                </div>

                <form onSubmit={async(e)=>{e.preventDefault(); await axios.put('/perfil', fP); alert("Perfil Actualizado"); setUser({...user, ...fP});}} className="space-y-6">
                    <input className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" value={fP.nombre} placeholder="Nombre Público" onChange={e=>setFP({...fP, nombre:e.target.value})} />
                    <input className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" value={fP.avatar_url} placeholder="URL Foto Perfil (https://...)" onChange={e=>setFP({...fP, avatar_url:e.target.value})} />
                    <div className="grid grid-cols-2 gap-6">
                        <input className="p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" value={fP.telefono} placeholder="Teléfono" onChange={e=>setFP({...fP, telefono:e.target.value})} />
                        <input className="p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" value={fP.direccion} placeholder="Dirección" onChange={e=>setFP({...fP, direccion:e.target.value})} />
                    </div>
            ...
            <div className="space-y-8 h-fit">
                <div className="bg-blue-600 p-12 rounded-[60px] text-white shadow-2xl relative overflow-hidden group">
                    ...
                    <p className="text-blue-100 font-bold text-sm leading-relaxed mb-10">¿Tienes dudas con el módulo de producción o nómina? Nuestro equipo de ingenieros está listo para ayudarte.</p>
                    <button onClick={()=>window.open('https://wa.me/573000000000')} className="bg-white text-blue-600 px-8 py-4 rounded-3xl font-black uppercase text-xs shadow-xl">Contactar por WhatsApp</button>
                </div>
                <div className="bg-white dark:bg-slate-900 p-10 rounded-[50px] border dark:border-slate-800 shadow-sm flex items-center justify-between group cursor-pointer">
                    <div className="flex items-center gap-6"><div className="w-16 h-16 bg-slate-50 dark:bg-slate-800 rounded-2xl flex items-center justify-center text-slate-400"><LifeBuoy size={32}/></div><p className="font-black text-xl dark:text-white">Documentación ERP</p></div>
...
function Login({ onLogin }) {
    const handle = async (e) => {
        ...
        if (res.data.success) onLogin(res.data); else alert("Error");
    };
    return (
        <div className="h-screen flex items-center justify-center bg-blue-600">
            <form onSubmit={handle} className="bg-white p-12 rounded-[60px] shadow-2xl w-full max-w-md text-center">
                <h1 className="text-4xl font-black mb-2 italic">AccuCloud.</h1>
                <p className="text-slate-400 font-bold text-[10px] uppercase mb-10 tracking-widest">Software de Gestión 2026</p>
                ...
                <button type="submit" className="w-full bg-slate-900 text-white font-black py-6 rounded-3xl shadow-xl uppercase">Ingresar al ERP</button>
            </form>
        </div>
    );
}
...
function ProduccionPro({ user }) {
    return <div className="p-20 bg-white dark:bg-slate-900 rounded-[60px] border-4 border-dashed text-center font-black text-slate-300 dark:text-slate-700 text-3xl uppercase">Módulo de Producción Sincronizando...</div>;
}