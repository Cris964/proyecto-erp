...
export default function App() {
  ...
  if (loadingSession) return <div className="h-screen flex items-center justify-center font-black text-blue-600 animate-pulse text-2xl uppercase italic tracking-tighter">AccuCloud Pro 2026 Cargando...</div>;
    ...
    return (
        <aside className="w-72 bg-slate-900 text-white flex flex-col p-6 shadow-2xl relative z-40">
            ...
            <nav className="flex-1 space-y-2 overflow-y-auto">
                {menuItems.filter(m => m.roles.includes(user?.cargo) || user?.cargo === 'Admin').map(m => (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    <div className="bg-white rounded-[50px] shadow-sm border overflow-hidden">
                        <table className="w-full text-left">
                            <thead className="bg-slate-50 text-[10px] font-black uppercase text-slate-400 border-b"><tr><th className="p-10">Funcionario</th><th>Salario</th><th className="text-center">Acción</th></tr></thead>
                            <tbody>{empleados.map(e=>(
                                <tr key={e.id} className="border-b hover:bg-slate-50 transition">
                                    ...
                                    <td className="text-center"><button onClick={()=>calcular(e)} className="p-4 bg-blue-50 text-blue-600 rounded-2xl hover:bg-blue-600 hover:text-white transition-all"><Calculator size={20}/></button></td>
                                </tr>
                            ))}</tbody>
                        </table>
                    </div>
                    {preview && (
                        <div className="bg-white p-12 rounded-[60px] shadow-2xl border-l-[25px] border-blue-600 animate-slide-up">
                            <h4 className="text-4xl font-black text-slate-800 tracking-tighter uppercase mb-8">{preview.nombre}</h4>
                            ...
                            <div className="bg-blue-600 p-12 rounded-[45px] text-center text-7xl font-black text-white shadow-xl mt-10 tracking-tighter leading-none">
                                {fmt(preview.neto)}
                            </div>
                            <button onClick={async ()=>{if(window.confirm("¿Enviar comprobante?")){await axios.post('/api/nomina/liquidar', preview); window.alert("Enviado.");}}} className="w-full py-8 bg-slate-900 text-white font-black rounded-[30px] shadow-xl mt-10 uppercase text-xs tracking-widest flex items-center justify-center gap-3"><Mail size={16}/> ENVIAR POR EMAIL</button>
                        </div>
                    )}
                </div>
            )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                    <div className="bg-white p-10 rounded-[50px] shadow-sm border h-fit">
                        ...
                        <form onSubmit={async (e)=>{e.preventDefault(); await axios.post('/api/produccion/materia', {...formM, company_id: user.company_id}); setFormM({nombre:'', unidad_medida:'mg', cantidad:0, costo:0}); load();}} className="space-y-5">
                            ...
                            <button className="w-full py-5 bg-slate-900 text-white font-black rounded-3xl shadow-xl hover:bg-black transition-all uppercase text-[10px] tracking-widest">Guardar en Almacén</button>
                        </form>
                    </div>
                    <div className="lg:col-span-2 bg-white rounded-[50px] shadow-sm border overflow-hidden">
                        <table className="w-full text-left">
                            <thead className="bg-slate-50 text-[10px] font-black uppercase border-b text-slate-400"><tr><th className="p-10">Material</th><th>Disponible</th><th>Costo unit.</th><th className="p-10 text-right">Valorizado</th></tr></thead>
            ...
            {sub === 'kits' && (
                <div className="bg-slate-900 p-20 rounded-[80px] shadow-2xl relative overflow-hidden text-white animate-slide-up">
                    ...
                    <div className="max-w-2xl relative z-10">
                        <h3 className="text-5xl font-black italic uppercase tracking-tighter text-blue-400 mb-6">Constructor de Kits</h3>
            ...
            {sub === 'ordenes' && (
                <div className="space-y-10">
                    <div className="bg-blue-600 p-16 rounded-[80px] text-white flex flex-col md:flex-row justify-between items-center shadow-2xl relative overflow-hidden">
                        ...
                        }} className="relative z-10 px-16 py-7 bg-white text-blue-600 font-black rounded-[40px] shadow-2xl hover:scale-110 transition-all uppercase text-sm tracking-widest active:scale-95 shadow-blue-900/50">+ Lanzar Orden OP</button>
                    </div>
                        ...
                        {ordenes.filter(o => ['Prealistamiento', 'Produccion'].includes(o.estado)).map(o => (
                            <div key={o.id} className="bg-white p-12 rounded-[70px] shadow-md border-l-[30px] border-blue-500 flex flex-col lg:flex-row justify-between items-center transition-all hover:shadow-2xl">
                                ...
                                <div className="flex gap-6 mt-10 lg:mt-0">
                                    {o.estado === 'Prealistamiento' && <button onClick={()=>handleAvanzar(o.id, 'Produccion')} className="flex items-center gap-4 px-12 py-6 bg-slate-900 text-white font-black rounded-[35px] shadow-2xl hover:bg-black transition-all uppercase text-xs active:scale-95"><Play size={24}/> Iniciar Etapa</button>}
                                    {o.estado === 'Produccion' && <button onClick={()=>handleAvanzar(o.id, 'Logistica')} className="flex items-center gap-4 px-12 py-6 bg-green-600 text-white font-black rounded-[35px] shadow-2xl hover:bg-green-700 transition-all uppercase text-xs active:scale-95"><CheckCircle size={24}/> Sellar Batch</button>}
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody>{ordenes.filter(o => o.estado === 'Logistica').map(o => (
                            <tr key={o.id} className="border-b hover:bg-slate-50 transition-all">
                                ...
                                <td><div className="text-xs text-slate-400 font-bold uppercase"><MapPin size={12} className="inline mr-1"/> Por asignar dirección...</div></td>
                                <td className="text-center p-10">
                                    <button onClick={async ()=>{ const d = window.prompt("Introduce Ciudad y Dirección de Despacho:"); if(d){ await axios.put(`/api/produccion/ordenes/${o.id}`, {estado: 'Cerrado'}); load(); window.alert("Batch Finalizado."); } }} className="px-10 py-4 bg-blue-600 text-white font-black rounded-3xl text-xs hover:bg-black transition-all">TERMINAR Y CERRAR</button>
                                </td>
                            </tr>
                        ))}</tbody>
                    </table>
...
function InventarioIndustrial({ user }) {
    ...
    const handleImportExcel = (e) => {
        ...
        reader.onload = async (evt) => {
            ...
            load(); window.alert("Importación finalizada.");
        };
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    <div className="bg-white p-12 rounded-[60px] shadow-sm border h-fit">
                        ...
                        <div className="flex gap-4">
                            ...
                            <button onClick={async ()=>{ const n=document.getElementById('nbn').value; await axios.post('/api/bodegas', {nombre: n, company_id: user.company_id}); load(); }} className="bg-blue-600 text-white px-10 rounded-3xl font-black shadow-xl hover:bg-black transition-all">AÑADIR</button>
                        </div>
                    </div>
                    <div className="bg-white rounded-[50px] shadow-sm border overflow-hidden">
                        <table className="w-full text-left">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 h-[750px] animate-fade-in">
            ...
            <div className="bg-slate-900 rounded-[70px] shadow-2xl p-12 text-white flex flex-col justify-between relative overflow-hidden">
                ...
                <div className="relative z-10 mt-10">
                    ...
                    <div className="text-8xl font-black tracking-tighter mb-12 text-white scale-x-90 origin-left">{fmt(total)}</div>
                    <button onClick={async ()=>{
        ...
        <div className="bg-white p-24 rounded-[90px] shadow-2xl text-center max-w-2xl mx-auto border-t-[30px] border-blue-600 animate-slide-up relative overflow-hidden">
            ...
            {turnoActivo ? (
                <div className="space-y-12">
                    ...
                    <button onClick={async ()=>{ if(window.confirm("¿Realizar arqueo y cerrar turno?")){ await axios.put('/api/turnos/finalizar', { turno_id: turnoActivo.id }); onUpdate(); } }} className="w-full py-8 bg-red-500 text-white font-black rounded-[40px] shadow-xl hover:bg-red-600 transition-all uppercase text-sm tracking-widest active:scale-95">Realizar Cierre de Turno</button>
                </div>
            ) : (
                <div className="space-y-10">
                    <p className="text-slate-400 font-bold px-20 text-xl italic tracking-tight opacity-60 uppercase">Módulo administrativo para operaciones de venta</p>
                    <button onClick={handleApertura} className="w-full py-10 bg-blue-600 text-white font-black rounded-[40px] shadow-2xl animate-bounce uppercase text-xs tracking-[5px] hover:bg-black transition-all">Aperturar con Clave Maestra</button>
                </div>
            )}
        </div>
    );
...
function ContabilidadView({ user }) {
    ...
    return (
        <div className="bg-white p-16 rounded-[80px] shadow-sm border border-slate-100 overflow-hidden pr-2 animate-fade-in">
            <h3 className="font-black text-3xl mb-12 italic text-blue-600 uppercase tracking-tighter flex items-center gap-5 underline decoration-blue-100 decoration-8 underline-offset-8"><Receipt size={40}/> Libro Diario de Ingresos</h3>
            <div className="overflow-x-auto">
                <table className="w-full text-left min-w-[900px]">
                    <thead className="bg-slate-50 text-[11px] font-black uppercase border-b tracking-[5px] text-slate-400"><tr><th className="p-10">Fecha / Hora</th><th>Cajero</th><th>Detalle</th><th className="text-right p-10">Monto Final</th></tr></thead>
                    <tbody>{datos.map(d=>(<tr key={d.id} className="border-b hover:bg-slate-50 transition-all">
                        ...
                        <td className="font-black text-slate-800 text-lg tracking-tight italic">Comprobante TPV #{d.id}</td>
        ...
        <div className="bg-white p-20 rounded-[80px] shadow-sm border h-[600px] relative">
             ...
             <ResponsiveContainer width="100%" height="80%">
                <BarChart data={chartData}>
                    ...
                    <Tooltip cursor={{fill: '#f8fafc'}} contentStyle={{borderRadius: '35px', border: 'none'}} />
    ...
    <div className="min-h-screen flex items-center justify-center bg-blue-600 p-4">
      <div className="bg-white p-20 rounded-[90px] shadow-2xl w-full max-w-lg border-t-[15px] border-slate-900 animate-slide-up">
        ...
        <form onSubmit={handleAuth} className="space-y-8">
          <input className="w-full p-7 bg-slate-100 rounded-[40px] font-black outline-none focus:ring-8 ring-blue-50 transition-all text-lg" placeholder="Email Corp." onChange={e => setEmail(e.target.value)} required />
          <input type="password" class="w-full p-7 bg-slate-100 rounded-[40px] font-black outline-none focus:ring-8 ring-blue-50 transition-all text-lg" placeholder="Password" onChange={e => setPassword(e.target.value)} required />
          <button className="w-full bg-slate-900 text-white font-black py-8 rounded-[40px] shadow-2xl hover:bg-blue-600 transition-all active:scale-95 uppercase text-sm mt-8 tracking-[5px]">Entrar</button>
        </form>
      </div>
    </div>
  );
