...
function VentasView({ user }) {
    ...
    return (
        <div className="grid grid-cols-3 gap-10 h-full animate-fade-in">
            ...
            <div className="bg-white dark:bg-slate-900 p-10 rounded-[60px] shadow-2xl border-t-[20px] border-blue-600 h-fit sticky top-0 border dark:border-slate-800 text-center">
                <h3 className="text-[10px] font-black uppercase text-slate-400 mb-8 tracking-widest">Facturación TPV</h3>
    ...
    return (
        <div className="space-y-10 animate-fade-in">
            <div className="flex gap-4 p-2 bg-white dark:bg-slate-900 rounded-[28px] w-fit shadow-lg border dark:border-slate-800">
                ...
                <button onClick={()=>setSub('new')} className={`px-10 py-3 rounded-2xl font-black text-xs ${sub==='new'?'bg-blue-600 text-white shadow-xl':'text-slate-400'}`}>AGREGAR PRODUCTO</button>
            ...
            {sub==='new' && (
                <form onSubmit={async(e)=>{e.preventDefault(); await axios.post('/api/productos', fP); alert("Sincronizado"); load(); setSub('list');}} className="bg-white dark:bg-slate-900 p-12 rounded-[60px] border dark:border-slate-800 shadow-2xl max-w-2xl mx-auto space-y-6">
                    ...
                    <select className="p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-black border-none dark:text-white" onChange={e=>setFP({...fP, bodega_id:e.target.value})}>
                        <option value="">Bodega destino...</option>{bodegas.map(b=><option key={b.id} value={b.id}>{b.nombre}</option>)}
                    </select>
                    <button type="submit" className="w-full py-6 bg-blue-600 text-white font-black rounded-3xl shadow-xl uppercase transition hover:scale-[1.02]">Registrar Producto</button>
                </form>
            )}
...
function ProduccionView({ user }) {
    ...
    const load = useCallback(() => {
        axios.get('/api/produccion/ordenes').then(res => setOrdenes(res.data));
        axios.get('/api/produccion/recetas').then(res => setRecetas(res.data));
    }, []);
    useEffect(() => { load(); }, [load]);
        ...
        <div className="space-y-8 animate-fade-in">
            ...
            {sub==='recetas' && (
                <div className="bg-white dark:bg-slate-900 p-12 rounded-[50px] border dark:border-slate-800 shadow-2xl">
                    <h3 className="text-2xl font-black italic mb-10 text-slate-800 dark:text-white text-center uppercase tracking-tighter">Recetas Farmacéuticas</h3>
                    <div className="grid grid-cols-2 gap-10">
                        {recetas.map(r => (
            ...
            {sub==='ordenes' && (
                <div className="space-y-10">
                    <div className="bg-white dark:bg-slate-900 p-10 rounded-[50px] border dark:border-slate-800 shadow-2xl flex gap-8 items-end border-t-[15px] border-blue-600">
                        <div className="flex-1 space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 ml-4">Medicamento</label><select id="rI" className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-black border-none dark:text-white">{recetas.map(r=><option key={r.id} value={r.id}>{r.nombre_kit}</option>)}</select></div>
    ...
    return (
        <div className="h-screen flex items-center justify-center bg-blue-600">
            <form onSubmit={handle} className="bg-white p-12 rounded-[60px] shadow-2xl w-full max-w-md text-center">
                <h1 className="text-4xl font-black mb-2 italic tracking-tighter text-slate-900 uppercase">AccuCloud<span className="text-blue-600">.</span></h1>
