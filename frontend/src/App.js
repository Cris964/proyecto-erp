...
import { LayoutDashboard, Package, ShoppingCart, Users, DollarSign, AlertTriangle, Wallet, Factory, ShieldCheck, Calculator, X, Plus } from 'lucide-react';
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <main className="flex-1 overflow-auto p-10 bg-slate-50">
        <header className="flex justify-between items-center mb-10">
            ...
            {turno ? <div className="px-6 py-2 bg-green-100 text-green-700 rounded-xl font-black text-xs">VENTAS TURNO: {fmt(turno.total_vendido)}</div> : <div className="px-6 py-2 bg-red-100 text-red-700 rounded-xl font-black text-xs uppercase">Caja Cerrada</div>}
        </header>
...
function Inventario() {
    ...
    return (
        <div className="space-y-6">
            ...
            {sub === 'new' && (
                <div className="bg-white p-10 rounded-[40px] border shadow-sm space-y-4 max-w-lg">
                    ...
                    <button onClick={async()=>{ await axios.post('/productos', form); alert("Guardado"); load(); setSub('list'); }} className="w-full bg-blue-600 text-white py-5 rounded-3xl font-black">GUARDAR EN NUBE</button>
                </div>
            )}
            {sub === 'bod' && (
                <div className="grid grid-cols-2 gap-8">
                    <div className="bg-white p-10 rounded-[40px] border h-fit space-y-4 shadow-sm text-center">
                        <h3 className="font-black italic">Nueva Bodega</h3>
                        <input className="w-full p-4 bg-slate-50 rounded-2xl border font-bold" placeholder="Nombre" value={formB.nombre} onChange={e=>setFormB({nombre:e.target.value})} />
                        <button onClick={async()=>{ await axios.post('/bodegas', formB); alert("Bodega Creada"); load(); setFormB({nombre:''}); }} className="w-full bg-slate-900 text-white py-4 rounded-3xl font-black">CREAR BODEGA</button>
                    </div>
            ...
            {sub === 'ins' && (
                <div className="grid grid-cols-3 gap-8">
                    <div className="bg-white p-8 rounded-[40px] border space-y-4 shadow-sm h-fit">
                        ...
                        <input className="w-full p-4 bg-slate-50 rounded-2xl font-bold border" placeholder="Stock" type="number" onChange={e=>setFormM({...formM, cantidad:e.target.value})} />
                        <input className="w-full p-4 bg-slate-50 rounded-2xl font-bold border" placeholder="Costo" type="number" onChange={e=>setFormM({...formM, costo:e.target.value})} />
                        <button onClick={async()=>{ await axios.post('/produccion/materia', formM); alert("Guardado"); load(); }} className="w-full bg-blue-600 text-white py-4 rounded-3xl font-black">GUARDAR</button>
                    </div>
                    <div className="col-span-2"><Table headers={['Materia Prima','Stock','Unidad','Costo']} data={materia} keys={['nombre','cantidad','unidad_medida','costo']} /></div>
                </div>
            )}
            {sub === 'rec' && (
                <div className="grid grid-cols-2 gap-8">
                    <div className="bg-white p-10 rounded-[40px] border space-y-4 shadow-sm h-fit">
                        ...
                        <input className="w-full p-4 bg-slate-50 rounded-2xl font-bold border" placeholder="Nombre Kit" value={formR.nombre_producto_final} onChange={e=>setFormR({...formR, nombre_producto_final:e.target.value})} />
                        <textarea className="w-full p-4 bg-slate-50 rounded-2xl font-bold border" placeholder="Descripción" onChange={e=>setFormR({...formR, descripcion:e.target.value})} />
                        <button onClick={async()=>{ await axios.post('/produccion/recetas', formR); alert("Kit Creado"); load(); }} className="w-full bg-slate-900 text-white py-4 rounded-3xl font-black shadow-xl">CREAR KIT MAESTRO</button>
                    </div>
...
function Nomina() {
    ...
    return (
        <div className="space-y-10">
            <div className="bg-white p-10 rounded-[50px] border shadow-sm grid grid-cols-3 gap-6">
                <input className="p-4 bg-slate-50 rounded-2xl border font-bold" placeholder="Nombre Completo" value={form.nombre} onChange={e=>setForm({...form, nombre:e.target.value})} />
                <input className="p-4 bg-slate-50 rounded-2xl border font-bold" placeholder="Documento" value={form.documento} onChange={e=>setForm({...form, documento:e.target.value})} />
                ...
                <button onClick={async()=>{ await axios.post('/empleados', form); alert("Vinculado"); load(); setForm({nombre:'',documento:'',salario:'',eps:'',arl:'',pension:''}); }} className="col-span-3 bg-blue-600 text-white py-5 rounded-3xl font-black uppercase text-xs">VINCULAR AHORA</button>
            </div>
            <div className="grid grid-cols-2 gap-6">
                {emps.map(e => (
                    <div key={e.id} className="bg-white p-8 rounded-[40px] border flex flex-col gap-4 shadow-sm">
                        ...
                        <p className="text-[10px] font-black text-slate-400 uppercase">DOC: {e.documento} | EPS: {e.eps} | ARL: {e.arl}</p>
                        <div className="flex justify-between items-center mt-auto border-t pt-4">
                            ...
                            <button onClick={()=>alert("Liquidando...")} className="bg-slate-900 text-white px-8 py-3 rounded-2xl font-black text-[10px] uppercase">Liquidar</button>
                        </div>
                    </div>
                ))}
            </div>
    ...
    return (
        <div className="space-y-10">
            <div className="grid grid-cols-3 gap-8 text-center">
                <div className="bg-white p-10 rounded-[45px] border shadow-sm"><p className="text-[10px] font-black uppercase text-slate-400 mb-2">Ingresos</p><h3 className="text-4xl font-black text-green-600">{fmt(ingresos)}</h3></div>
                <div className="bg-white p-10 rounded-[45px] border shadow-sm"><p className="text-[10px] font-black uppercase text-slate-400 mb-2">Egresos</p><h3 className="text-4xl font-black text-red-500">{fmt(egresos)}</h3></div>
                <div className="bg-slate-900 p-10 rounded-[45px] shadow-2xl text-white"><p className="text-[10px] font-black uppercase text-slate-500 mb-2">Balance</p><h3 className="text-4xl font-black">{fmt(ingresos - egresos)}</h3></div>
            </div>
...
function CajaView({ user, turno, onUpdate }) {
    ...
    return (
        <div className="flex flex-col items-center justify-center p-20 bg-white rounded-[60px] border shadow-2xl max-w-lg mx-auto flex flex-col items-center mt-10">
            <div className="w-24 h-24 bg-blue-50 text-blue-600 rounded-[35px] flex items-center justify-center mb-10"><Wallet size={48}/></div>
            {turno ? (
                <button onClick={async()=>{ await axios.put('/turnos/finalizar', {turno_id: turno.id}); onUpdate(); alert("Cerrado"); }} className="w-full py-6 bg-red-500 text-white font-black rounded-3xl shadow-xl">REALIZAR CIERRE DE CAJA</button>
            ) : (
                <div className="w-full space-y-8 text-center">
                    ...
                    <button onClick={async()=>{ await axios.post('/turnos/iniciar', {base_caja: base}); onUpdate(); alert("Iniciado"); }} className="w-full py-6 bg-blue-600 text-white font-black rounded-3xl shadow-xl uppercase text-xs">Abrir Caja en Nube</button>
                </div>
            )}
        </div>
    );
    ...
    return (
        <div className="space-y-10">
            <div className="bg-white p-10 rounded-[40px] border flex gap-4 max-w-4xl shadow-sm">
                ...
                <button onClick={async()=>{ await axios.post('/admin/usuarios', form); alert("Guardado"); load(); setForm({nombre:'',email:'',password:'',cargo:'Vendedor'}); }} className="bg-blue-600 text-white px-10 rounded-2xl font-black">GUARDAR</button>
            </div>
            <Table headers={['Funcionario','Email','Rol']} data={users} keys={['nombre','email','cargo']} />
        </div>
    );
}
