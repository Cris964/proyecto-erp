            ...
            {sub==='new' && (
                <form onSubmit={handleSave} className="bg-white dark:bg-slate-900 p-12 rounded-[60px] border dark:border-slate-800 shadow-2xl max-w-2xl mx-auto space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        ...
                        <div className="space-y-1"><label className="text-[9px] font-black uppercase text-slate-400 ml-4">Fecha Vencimiento</label><input className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" type="date" onChange={e=>setFP({...fP, fecha_vencimiento:e.target.value})} required/></div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-10">
                        {recetas.map(r => (
                            <div key={r.id} className="p-10 bg-slate-50 dark:bg-slate-800 rounded-[45px] border-2 border-dashed border-slate-200 dark:border-slate-700">
                                ...
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b dark:border-slate-700 pb-2 mb-4">Materia Prima Vinculada</p>
                                <div className="text-xs font-bold text-slate-500 italic">--- Lista de componentes vinculados ---</div>
                            </div>
                        ))}
                    ...
                    {ordenes.map(o => (
                        <div key={o.id} className="bg-white dark:bg-slate-900 p-10 rounded-[45px] border dark:border-slate-800 shadow-sm flex justify-between items-center group hover:border-blue-500 transition-all">
                            <div>
                                ...
                                <p className="text-[10px] font-bold text-slate-400 uppercase mt-3"><span>Montó: {o.usuario_monta}</span>{o.bodeguero_alista && <span className="text-green-500 ml-4">Alistó: {o.bodeguero_alista}</span>}</p>
                            </div>
                            <div className="flex items-center gap-8">
                                <div className="text-right">
                                    <p className="text-[10px] font-black uppercase text-slate-400 mb-1">Requerido / Salida</p>
                                    <p className="font-black text-2xl dark:text-white">{o.cantidad_requerida} / <span className="text-blue-600">{o.cantidad_salida}</span></p>
                                </div>
                ...
                {emps.map(e => (
                    <div key={e.id} className="bg-white dark:bg-slate-900 p-10 rounded-[45px] border dark:border-slate-800 shadow-sm group hover:border-blue-600 transition-all flex flex-col gap-6">
                        ...
                        <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-800 p-6 rounded-[30px]">
                            ...
                            <button onClick={()=>alert(`Liquidando a ${e.nombre}... Salario Mes: ${fmt(e.valor_dia * 30 + 162000)}`)} className="bg-slate-900 dark:bg-blue-600 text-white px-8 py-3 rounded-2xl font-black text-[10px] uppercase shadow-lg">LIQUIDAR</button>
                        </div>
                    </div>
                ))}
            </div>
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 animate-fade-in">
            ...
            <div className="bg-blue-600 p-12 rounded-[60px] text-white shadow-2xl h-fit">
                ...
                <p className="text-blue-100 font-bold text-sm leading-relaxed mb-10">Nuestro equipo de ingenieros está disponible para ayudarte con la configuración de tu empresa o dudas técnicas sobre los módulos industriales.</p>
            ...
            <div className="bg-white dark:bg-slate-900 p-20 rounded-[60px] border dark:border-slate-800 shadow-2xl border-t-[20px] border-blue-600">
                ...
                {activo ? (
                    <div className="space-y-8 animate-fade-in text-left">
                        ...
                        <div className="grid grid-cols-2 gap-6">
                            <div className="bg-slate-50 dark:bg-slate-800 p-8 rounded-[40px] text-left"><p className="text-[10px] font-black text-slate-400 uppercase mb-2">Base</p><p className="text-3xl font-black text-slate-600 dark:text-white">{fmt(activo.base_caja)}</p></div>
                            <div className="bg-blue-50 dark:bg-blue-900/20 p-8 rounded-[40px] text-left border border-blue-100 dark:border-blue-900"><p className="text-[10px] font-black text-blue-400 uppercase mb-2">Ventas</p><p className="text-3xl font-black text-blue-600">{fmt(activo.total_vendido)}</p></div>
                        </div>
                        <button onClick={async()=>{ if(confirm("¿Cerrar?")){ await axios.put('/api/turnos/finalizar', {turno_id: activo.id}); load(); }}} className="w-full py-7 bg-red-500 text-white font-black rounded-3xl shadow-xl uppercase">Realizar Cierre Mayor</button>
                    </div>
                ) : (
    ...
    return (
        <div className="h-screen flex items-center justify-center bg-blue-600">
            <form onSubmit={handle} className="bg-white p-12 rounded-[60px] shadow-2xl w-full max-w-md text-center">
                ...
                <p className="text-slate-400 font-bold text-[10px] uppercase mb-10 tracking-widest">SaaS ERP 2026</p>
...
function PagosView() {
    return <div className="p-20 bg-white dark:bg-slate-900 rounded-[60px] border-4 border-dashed text-center font-black text-slate-300 dark:text-slate-700 text-3xl uppercase">Sincronizando Pagos...</div>;
}