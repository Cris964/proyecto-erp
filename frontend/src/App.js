      ...
      <div className="flex h-screen bg-slate-50 dark:bg-slate-950 font-sans overflow-hidden transition-all duration-300">
        ...
        <aside className="w-80 bg-white dark:bg-slate-900 border-r dark:border-slate-800 flex flex-col p-8 shadow-2xl z-20">
          <div className="mb-10 flex items-center justify-between">
            ...
            <button onClick={toggleTheme} className="p-2 bg-slate-100 dark:bg-slate-800 rounded-xl text-slate-500">
          ...
          <div className="pt-6 border-t dark:border-slate-800 mt-6 flex flex-col gap-4">
              ...
              <button onClick={()=>{localStorage.clear(); window.location.reload();}} className="text-red-500 font-black text-xs uppercase p-4 rounded-2xl w-full hover:bg-red-50 dark:hover:bg-red-900/20">Cerrar Sesión</button>
          </div>
        </aside>
                ...
                <div className="bg-white dark:bg-slate-900 p-10 rounded-[50px] shadow-sm border dark:border-slate-800 h-[450px] overflow-auto">
                    ...
                    <div className="space-y-4">
                        {data.topProducts.map((p, i) => (
                            <div key={i} className="flex justify-between items-center p-5 bg-slate-50 dark:bg-slate-800 rounded-[30px] border dark:border-slate-700">
                                <span className="font-black text-slate-800 dark:text-white">{p.nombre_producto}</span>
                                <span className="bg-blue-600 text-white px-5 py-1 rounded-full font-black text-[10px] uppercase">{p.c} Uds</span>
                            </div>
                        ))}
                    </div>
                </div>
...
function VentasView({ user }) {
    ...
    if(!turno) return <div className="p-20 bg-white dark:bg-slate-900 rounded-[60px] border-4 border-dashed text-center font-black text-slate-300 dark:text-slate-700 text-4xl uppercase">Abre turno en Cajas para facturar</div>;
...
function InventarioView() {
    ...
    const [f, setF] = useState({ sku:'', nombre:'', fecha_vencimiento:'', costo_compra:0, costo_venta:0, stock:0, min_stock:5, bodega_id:'' });
    ...
    const handleSave = async (e) => {
        e.preventDefault();
        await axios.post('/api/productos', f);
    ...
    return (
        <div className="space-y-10">
            ...
            {sub==='list' && (
                <div className="bg-white dark:bg-slate-900 rounded-[50px] border dark:border-slate-800 shadow-2xl overflow-hidden">
            ...
            {sub==='new' && (
                <form onSubmit={handleSave} className="bg-white dark:bg-slate-900 p-12 rounded-[60px] border dark:border-slate-800 shadow-2xl max-w-2xl mx-auto space-y-6">
                    ...
                    <input className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" placeholder="Nombre Comercial" onChange={e=>setF({...f, nombre:e.target.value})} required/>
                    <input className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" placeholder="Código SKU" onChange={e=>setF({...f, sku:e.target.value})} required/>
                    <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-1"><label className="text-[9px] font-black uppercase text-slate-400 ml-4">Costo de Compra</label><input className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" type="number" onChange={e=>setF({...f, costo_compra:e.target.value})} required/></div>
                        <div className="space-y-1"><label className="text-[9px] font-black uppercase text-slate-400 ml-4">Precio de Venta</label><input className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" type="number" onChange={e=>setF({...f, costo_venta:e.target.value})} required/></div>
                    </div>
                    <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-1"><label className="text-[9px] font-black uppercase text-slate-400 ml-4">Stock Inicial</label><input className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" type="number" onChange={e=>setF({...f, stock:e.target.value})} required/></div>
                        <div className="space-y-1"><label className="text-[9px] font-black uppercase text-slate-400 ml-4">Fecha Vencimiento</label><input className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" type="date" onChange={e=>setF({...f, fecha_vencimiento:e.target.value})} required/></div>
                    </div>
                    <button type="submit" className="w-full py-6 bg-blue-600 text-white font-black rounded-3xl shadow-xl uppercase">Guardar en Nube</button>
                </form>
            )}
    ...
    return (
        <div className="space-y-8">
            <div className="flex gap-4 p-2 bg-white dark:bg-slate-900 rounded-[28px] w-fit shadow-lg border dark:border-slate-800">
                ...
                <button onClick={()=>setSub('ordenes')} className={`px-10 py-3 rounded-2xl font-black text-xs ${sub==='ordenes'?'bg-blue-600 text-white shadow-xl':'text-slate-400'}`}>FLUJO DE PRODUCCIÓN</button>
            </div>

            {sub==='recetas' && (
                <div className="bg-white dark:bg-slate-900 p-12 rounded-[50px] border dark:border-slate-800 shadow-2xl">
                    <h3 className="text-2xl font-black italic mb-10 text-slate-800 dark:text-white">Kits Farmacéuticos</h3>
                    <div className="grid grid-cols-2 gap-10">
                        {recetas.map(r => (
                            <div key={r.id} className="p-10 bg-slate-50 dark:bg-slate-800 rounded-[45px] border-2 border-dashed border-slate-200 dark:border-slate-700">
                                ...
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b dark:border-slate-700 pb-2 mb-4">Componentes del Kit</p>
                                <div className="space-y-3"><div className="flex justify-between items-center text-xs font-bold text-slate-500 italic">--- Listado de insumos vinculados ---</div></div>
                            </div>
                        ))}
                        <button onClick={async()=>{ const n = prompt("Nombre del Kit?"); await axios.post('/api/produccion/recetas', {nombre:n, descripcion:'...'}); load(); }} className="p-10 border-4 border-dotted rounded-[45px] flex flex-col items-center justify-center text-slate-300 font-black hover:bg-slate-50 dark:hover:bg-slate-800 transition-all uppercase">+ Crear Receta Maestra</button>
                    </div>
                </div>
            )}
            ...
            {sub==='ordenes' && (
                <div className="space-y-10">
                    <div className="bg-white dark:bg-slate-900 p-10 rounded-[50px] border dark:border-slate-800 shadow-2xl flex gap-8 items-end border-t-[15px] border-blue-600">
                        <div className="flex-1 space-y-1"><label className="text-[10px] font-black uppercase text-slate-400 ml-4">Medicamento a fabricar</label><select id="rI" className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-black border-none dark:text-white">{recetas.map(r=><option key={r.id} value={r.id}>{r.nombre_kit}</option>)}</select></div>
                    ...
                    {ordenes.map(o => (
                        <div key={o.id} className="bg-white dark:bg-slate-900 p-10 rounded-[45px] border dark:border-slate-800 shadow-sm flex justify-between items-center group hover:border-blue-500 transition-all">
                            <div>
                                <h4 className="text-2xl font-black text-slate-900 dark:text-white italic uppercase">{o.numero_orden} - {o.nombre_kit}</h4>
                                <div className="flex gap-4 mt-3 text-[9px] font-bold text-slate-400 uppercase"><span>Montó: {o.quien_monta}</span>{o.bodeguero_alista && <span className="text-green-500">Alistó: {o.bodeguero_alista}</span>}{o.operario_produccion && <span className="text-blue-500">Procesó: {o.operario_produccion}</span>}</div>
                            </div>
                            <div className="flex items-center gap-8 text-center">
                                <div><p className="text-[9px] font-black uppercase text-slate-400">Requerido / Salida</p><p className="font-black text-2xl dark:text-white">{o.cantidad_requerida} / <span className="text-blue-600">{o.cantidad_salida}</span></p></div>
                                ...
                                {o.estado==='Alistada' && <button onClick={()=>{ const out = prompt("Salida Real?"); const obs = prompt("Obs?"); avanzar(o.id, 'Procesada', out, obs); }} className="bg-orange-500 text-white px-8 py-3 rounded-2xl font-black text-[10px] uppercase">Procesar Orden</button>}
...
function NominaView() {
    ...
    const [f, setF] = useState({ nombre:'', documento:'', valor_dia:0, hire_date:'', eps:'', arl:'', pension:'' });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            <form onSubmit={async(e)=>{e.preventDefault(); await axios.post('/api/empleados', f); alert("Vinculado"); load(); e.target.reset();}} className="bg-white dark:bg-slate-900 p-12 rounded-[50px] border dark:border-slate-800 shadow-2xl space-y-6 h-fit">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <input className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl font-bold border-none dark:text-white" type="number" placeholder="Valor Día $" onChange={e=>setF({...f, valor_dia:e.target.value})}/>
                    <input className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl font-bold border-none dark:text-white" type="date" onChange={e=>setF({...f, hire_date:e.target.value})}/>
                </div>
                ...
                {emps.map(e => (
                    <div key={e.id} className="bg-white dark:bg-slate-900 p-10 rounded-[45px] border dark:border-slate-800 shadow-sm group hover:border-blue-600 transition-all flex flex-col gap-6">
                        <div className="flex justify-between items-center border-b dark:border-slate-800 pb-6">
                            <div><p className="font-black text-2xl dark:text-white leading-none">{e.nombre}</p><p className="text-[10px] font-black text-slate-400 uppercase mt-2">DOC: {e.documento}</p></div>
                        ...
                        <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-800 p-6 rounded-[30px]">
                            ...
                            <button onClick={()=>alert(`Liquidando a ${e.nombre}...\nTotal Mes (30d): ${fmt(e.valor_dia * 30 + 162000)}`)} className="bg-slate-900 dark:bg-blue-600 text-white px-8 py-3 rounded-2xl font-black text-[10px] uppercase shadow-lg">LIQUIDAR</button>
                        </div>
                    </div>
                ))}
            </div>
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            <form onSubmit={async(e)=>{e.preventDefault(); await axios.post('/api/pagos', f); alert("Registrado"); load(); e.target.reset();}} className="bg-white dark:bg-slate-900 p-12 rounded-[50px] border dark:border-slate-800 shadow-2xl space-y-6 h-fit">
                <h3 className="text-2xl font-black italic text-slate-800 dark:text-white">Registrar Egreso</h3>
                ...
                <textarea className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white" placeholder="Motivo detallado del pago..." onChange={e=>setF({...f, descripcion:e.target.value})}/>
            ...
            <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-[50px] border dark:border-slate-800 shadow-sm overflow-hidden h-fit">
                <table className="w-full text-left">
                    <thead className="bg-slate-50 dark:bg-slate-800 text-[10px] font-black uppercase tracking-widest border-b dark:border-slate-700"><tr><th className="p-8">Registro de Pago</th><th>Categoría</th><th>Beneficiario</th><th>Monto</th></tr></thead>
                    <tbody>{data.map(p => (
...
function CajaView({ user }) {
    ...
    const load = useCallback(() => {
        axios.get('/api/caja-menor').then(res => setMenor(res.data));
    ...
    return (
        <div className="space-y-10 text-center">
            <div className="flex gap-4 p-2 bg-white dark:bg-slate-900 rounded-[25px] w-fit shadow-lg border dark:border-slate-800 mx-auto">
                ...
                <button onClick={()=>setSub('menor')} className={`px-10 py-3 rounded-2xl font-black text-xs ${sub==='menor'?'bg-blue-600 text-white shadow-xl':'text-slate-400'}`}>CAJA MENOR / TARJETAS</button>
            </div>
            ...
            {sub==='mayor' && (
                <div className="max-w-2xl mx-auto mt-10">
                    <div className="bg-white dark:bg-slate-900 p-20 rounded-[60px] border dark:border-slate-800 shadow-2xl border-t-[20px] border-blue-600">
                        <div className="w-24 h-24 bg-blue-50 dark:bg-slate-800 text-blue-600 rounded-[40px] flex items-center justify-center mx-auto mb-10 shadow-inner"><Wallet size={54}/></div>
                        {activo ? (
                            <div className="space-y-8 animate-fade-in text-left">
                                <h3 className="text-3xl font-black italic dark:text-white text-center">Caja Central Activa</h3>
                                <div className="grid grid-cols-2 gap-6">
                                    <div className="bg-slate-50 dark:bg-slate-800 p-8 rounded-[40px] border dark:border-slate-700"><p className="text-[10px] font-black text-slate-400 uppercase mb-2">Base</p><p className="text-3xl font-black dark:text-white">{fmt(activo.base_caja)}</p></div>
                                    <div className="bg-blue-50 dark:bg-blue-900/20 p-8 rounded-[40px] border border-blue-100 dark:border-blue-900"><p className="text-[10px] font-black text-blue-400 uppercase mb-2">Vendido</p><p className="text-3xl font-black text-blue-600">{fmt(activo.total_vendido)}</p></div>
                                </div>
                                <button onClick={async()=>{ if(confirm("¿Cerrar?")){ await axios.put('/api/turnos/finalizar', {turno_id: activo.id}); load(); }}} className="w-full py-7 bg-red-500 text-white font-black rounded-3xl shadow-xl uppercase">Realizar Cierre de Caja Mayor</button>
                            </div>
                        ) : (
                            <div className="space-y-8 animate-fade-in text-center">
                                <h3 className="text-3xl font-black italic dark:text-white">Apertura de Jornada</h3>
                                <p className="text-slate-400 text-sm font-bold px-10">Ingresa el fondo inicial en efectivo para comenzar a facturar hoy.</p>
                                ...
                                <button onClick={async()=>{ await axios.post('/api/turnos/iniciar', {base_caja: document.getElementById('bC').value}); load(); }} className="w-full py-7 bg-blue-600 text-white font-black rounded-3xl shadow-xl uppercase">Iniciar Turno Cloud</button>
                            </div>
                        )}
                    </div>
                </div>
            ...
            {sub==='menor' && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                    <form onSubmit={async(e)=>{e.preventDefault(); await axios.post('/api/caja-menor', {tipo:e.target.t.value, tarjeta:e.target.tj.value, monto:e.target.m.value, descripcion:e.target.d.value}); alert("Registrado"); load(); e.target.reset();}} className="bg-white dark:bg-slate-900 p-12 rounded-[50px] border dark:border-slate-800 shadow-2xl space-y-6 h-fit">
                        <h3 className="text-xl font-black italic dark:text-white">Gasto Caja Menor</h3>
                        <select name="t" className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl font-black border-none outline-none dark:text-white"><option value="Efectivo">Efectivo</option><option value="Tarjeta">Tarjeta (Debito/Credito)</option></select>
                        <input name="tj" className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl font-bold dark:text-white" placeholder="Banco / Franquicia" />
                        <input name="m" className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl font-bold dark:text-white" type="number" placeholder="Monto $" required/>
                        <textarea name="d" className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl font-bold dark:text-white" placeholder="Descripción" required/>
                        <button type="submit" className="w-full py-5 bg-slate-900 dark:bg-blue-600 text-white font-black rounded-3xl shadow-xl">GUARDAR MOVIMIENTO</button>
                    </form>
                    <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-[50px] border dark:border-slate-800 shadow-sm overflow-hidden h-fit">
                         <table className="w-full text-left">
                            <thead className="bg-slate-50 dark:bg-slate-800 text-[10px] font-black uppercase tracking-widest border-b dark:border-slate-700"><tr><th className="p-8">Fecha</th><th>Tipo</th><th>Monto</th><th>Detalle</th></tr></thead>
                            <tbody>{menor.map(m => (
                                <tr key={m.id} className="border-b dark:border-slate-800 transition">
                                    <td className="p-8 text-[9px] font-black text-slate-400">{new Date(m.fecha).toLocaleString()}</td>
                                    <td className="font-bold text-slate-700 dark:text-white">{m.tipo}</td>
                                    <td className="font-black text-blue-600">{fmt(m.monto)}</td>
                                    <td className="text-xs font-bold text-slate-400">{m.detalle_tarjeta || '---'}</td>
                                </tr>
                            ))}</tbody>
                        </table>
                    </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 animate-fade-in h-fit">
            <div className="bg-white dark:bg-slate-900 p-12 rounded-[60px] border dark:border-slate-800 shadow-2xl space-y-10">
                ...
                <form onSubmit={async(e)=>{e.preventDefault(); await axios.put('/api/perfil', f); alert("Perfil Actualizado"); setUser({...user, ...f}); localStorage.setItem('erp_user', JSON.stringify({...user, ...f}));}} className="space-y-6">
                    ...
                    <input className="w-full p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white outline-none" value={f.avatar_url} placeholder="URL Foto de Perfil (https://...)" onChange={e=>setF({...f, avatar_url:e.target.value})} />
                    <div className="grid grid-cols-2 gap-6">
                        <input className="p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white outline-none" value={f.telefono} placeholder="Teléfono Móvil" onChange={e=>setF({...f, telefono:e.target.value})} />
                        <input className="p-5 bg-slate-50 dark:bg-slate-800 rounded-3xl font-bold border-none dark:text-white outline-none" value={f.direccion} placeholder="Dirección Oficina" onChange={e=>setF({...f, direccion:e.target.value})} />
                    </div>
                    <button type="submit" className="w-full py-6 bg-slate-900 dark:bg-blue-600 text-white font-black rounded-[30px] shadow-xl uppercase">Guardar Cambios</button>
                </form>
            </div>
            <div className="space-y-8 h-fit">
                <div className="bg-blue-600 p-12 rounded-[60px] text-white shadow-2xl relative overflow-hidden group">
                    <div className="absolute top-0 right-0 w-40 h-40 bg-white/10 rounded-bl-full translate-x-10 -translate-y-10 group-hover:scale-110 transition-transform"></div>
                    <h3 className="text-2xl font-black italic mb-4">Soporte Técnico Especializado</h3>
                    <p className="text-blue-100 font-bold text-sm leading-relaxed mb-10">Nuestro equipo de ingenieros está disponible 24/7 para ayudarte con la configuración de tu empresa.</p>
                    <button onClick={()=>window.open('https://wa.me/57321654987')} className="bg-white text-blue-600 px-8 py-4 rounded-3xl font-black uppercase text-xs shadow-xl">Contactar WhatsApp</button>
                </div>
            </div>
        </div>
    );
